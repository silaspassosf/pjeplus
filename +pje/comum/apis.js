   ...
   */
    async executar(dominio, params = {}, fetchOptions = {}, extraOptions = { replaceDev015: true, carregarRespostaJson: true}) {
      ...
      try {
        ...
        console.error(`Erro ao executar ${this.method} ${url}:`, erro);
        throw erro;
      }
      if (!resposta.ok || (resposta.status >= 400 )) {
        console.error(`Erro HTTP ${resposta.status} ao executar ${this.method} ${url}`);
        return null;
...
const apis = {
    ...
    idTipoDocumentoAcordao: new ApiWrapper(
      '/pje-comum-api/api/parametros/PARAMETRO_ID_TIPO_ACORDAO/restricao',
      {}
    ),
