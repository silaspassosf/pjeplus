...
from .judicial import make_ato_wrapper
...
def _inserir_relatorio_conciso_sisbajud(driver, numero_processo=None, conteudo_relatorio=None, debug=True):
    ...
    try:
        ...
        if conteudo_relatorio:
            if debug:
                print(f'[ATO_BLOQ] Usando conteúdo fornecido diretamente: {len(conteudo_relatorio)} caracteres')
            conteudo = conteudo_relatorio
        else:
            # Ler do arquivo específico PEC/sisbajud_conciso_ultimo.txt
            if debug:
                print(f'[ATO_BLOQ] Buscando relatório conciso do arquivo PEC/sisbajud_conciso_ultimo.txt')
            ...
            if not os.path.exists(arquivo_conciso):
                if debug:
                    print(f'[ATO_BLOQ] ❌ Arquivo não encontrado: {arquivo_conciso}')
                return False
            ...
            if debug:
                print(f'[ATO_BLOQ] ✅ Relatório conciso lido: {len(conteudo)} caracteres')
        
        if not conteudo:
            if debug:
                print('[ATO_BLOQ] ❌ Nenhum conteúdo SISBAJUD encontrado')
            return False
        
        if debug:
            print(f'[ATO_BLOQ] ✅ Relatório encontrado: {len(conteudo)} caracteres')
            print(f'[ATO_BLOQ] Inserindo no marcador "--"...')
        ...
        if debug:
            print(f'[ATO_BLOQ] Resultado da inserção: {"✅ Sucesso" if resultado else "❌ Falha"}')
        
        return resultado
        
    except Exception as e:
        if debug:
            print(f'[ATO_BLOQ] ❌ Erro ao inserir relatório conciso: {e}')
            import traceback
            traceback.print_exc()
        return False
