...
"""
@module: Fix.selenium_base.element_interaction
@responsibility: Interação com elementos DOM (clicar, preencher, scroll)
@depends_on: selenium.webdriver, Fix.selenium_base.wait_operations
@used_by: Todos os módulos que interagem com elementos
@entry_points: safe_click, preencher_campo, preencher_campos_prazo, preencher_multiplos_campos, _clicar_botao_movimentar, _clicar_botao_tarefa_processo, js_base
@tags: #selenium #click #fill #interaction #forms
@created: 2026-01-29
@refactored_from: Fix/core.py lines 105-286, 463-607, 973-1082, 2766-2846
"""
...
from .wait_operations import esperar_elemento
...
def safe_click(driver: WebDriver, selector_or_element: Union[str, WebElement], timeout: int = 10, by: By = None, log: bool = False) -> bool:
    """
    ...
    Args:
        ...
        selector_or_element: Seletor CSS/XPATH (string) ou WebElement
        timeout: Timeout em segundos (default: 10)
        by: Tipo de seletor (default: None)
        log: Se True, loga operação (default: False)
        
    Returns:
        True se clicou com sucesso, False caso contrário
    """
    # Import wait function to avoid circular dependency
    from .wait_operations import wait
    ...
    if element and element.is_displayed():
        ...
        # Se o elemento não estiver visível, tentar um click via JS
        try:
            driver.execute_script("arguments[0].click();", element)
            if DEBUG:
...
def preencher_campo(driver: WebDriver, seletor: str, valor: Union[str, int], trigger_events: bool = True, limpar: bool = True, log: bool = False) -> bool:
    ...
    Args:
        ...
        trigger_events: Se True, dispara input/change/blur (default: True)
        limpar: Se True, limpa campo antes de preencher (default: True)
...
def preencher_campos_prazo(driver: WebDriver, valor: int = 0, timeout: int = 10, log: bool = True) -> bool:
    ...
    Args:
        driver: WebDriver Selenium
        valor: Valor do prazo em dias (default: 0)
        timeout: Timeout em segundos (default: 10)
        log: Se True, loga operação (default: True)
...
def preencher_multiplos_campos(driver: WebDriver, campos_dict: dict, log: bool = False) -> dict:
    """
    ...
    try:
        ...
        if log:
            for _seletor, _sucesso in resultado.items():
                pass
...
def _clicar_botao_movimentar(driver: WebDriver, timeout: int = 10, log: bool = False) -> bool:
    """
...
def _clicar_botao_tarefa_processo(driver: WebDriver, timeout: int = 10, log: bool = False) -> bool:
    """
...
def _tentar_click_padrao(driver: WebDriver, element: WebElement, log: bool, attempt: int) -> bool:
    """
...
def _tentar_click_javascript(driver: WebDriver, element: WebElement, log: bool) -> bool:
    """
...
__all__ = [
    ...
    '_clicar_botao_movimentar',
    '_clicar_botao_tarefa_processo',
    '_tentar_click_padrao',
    '_tentar_click_javascript',
    '_tentar_click_actionchains',
    '_tentar_click_javascript_avancado'
]
