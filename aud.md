...
var diaDaSemana = new Date().getDay(); // 0 = Dom, 1 = Seg, 2 = Ter, 3 = Qua, 4 = Qui, 5 = Sex, 6 = Sáb
        ...
        function renderizarPainel(perfilId, estaRetraido) {
            ...
            // ── Julgamento designado (Inst LLM / Encerramento s/ oitiva) ──────
            // Data: sempre a SEGUNDA sexta-feira seguinte à ativação do botão.
            // Hora: começa em 13h00 e avança 5 min a cada ativação, com a sequência
            // compartilhada entre os dois botões (localStorage).
            // Reset automático: quando a designação muda (passou a sexta), a data
            // calculada é outra e o horário volta para 13h00.
            var JULG_SEQ_KEY = 'aud_julgamento_seq';
            var JULG_DATA_KEY = 'aud_julgamento_data';

            function segundaSextaSeguinte() {
                var d = new Date();
                d.setHours(0, 0, 0, 0);
                var ateSexta = (5 - d.getDay() + 7) % 7 || 7;  // próxima sexta (nunca hoje)
                d.setDate(d.getDate() + ateSexta + 7);         // e a segunda sexta seguinte
                return String(d.getDate()).padStart(2, '0') + '/' +
                       String(d.getMonth() + 1).padStart(2, '0') + '/' + d.getFullYear();
            }

            function horaJulgamento(dataJulgamento) {
                var seq = 0;
                try {
                    var seqSalva = parseInt(localStorage.getItem(JULG_SEQ_KEY) || '0', 10) || 0;
                    var dataSalva = localStorage.getItem(JULG_DATA_KEY) || '';
                    seq = (dataSalva === dataJulgamento) ? seqSalva : 0;  // data nova → 13h00
                } catch (e) { seq = 0; }
                var total = 13 * 60 + seq * 5;
                try {
                    localStorage.setItem(JULG_SEQ_KEY, String(seq + 1));
                    localStorage.setItem(JULG_DATA_KEY, dataJulgamento);
                } catch (e) { /* sem persistência */ }
                return String(Math.floor(total / 60)).padStart(2, '0') + 'h' + String(total % 60).padStart(2, '0');
            }

            // Marcadores %JDATA% / %JHORA% (designação de julgamento): expandidos no
            // clique. Só consome a sequência de horários se o template usar os marcadores.
            function completarJulgamento(html) {
                var texto = String(html || '');
                if (texto.indexOf('%JDATA%') === -1 && texto.indexOf('%JHORA%') === -1) return texto;
                var data = segundaSextaSeguinte();
                return texto.split('%JDATA%').join(data)
                            .split('%JHORA%').join(horaJulgamento(data));
            }
...
<p style=\"text-align:justify;text-indent:3cm;\">Designa-se audiência de JULGAMENTO para o dia %JDATA% às %JHORA% horas, com intimação via diário oficial.</p>\
<p style=\"text-align:justify;text-indent:3cm;\">Audiência encerrada às %HEC%.</p>\
<p style=\"text-align:justify;text-indent:3cm;\">Registra-se que magistrado, partes e advogados estão no fórum, presencialmente.</p>\
...
<p style=\"text-align:justify;text-indent:3cm;\">Designa-se para julgamento a data de %JDATA% às %JHORA%, sendo que as partes serão intimadas da sentença via Diário Oficial.</p>\
<p style=\"text-align:justify;text-indent:3cm;\">Audiência encerrada às %HEC%.</p>\
<p style=\"text-align:justify;text-indent:3cm;\">Registra-se que magistrado, partes e advogados estão no fórum, presencialmente.</p>\
...
<p style=\"text-align:justify;text-indent:3cm;\">Tendo em vista o pedido de adicional de INSALUBRIDADE, determina-se a realização da perícia técnica. Fica nomeada a Sra. Perita REGIANE SOUZA ROCHA SILVA - e-mail: peritaregianesilva@gmail.com, devendo apresentar seu laudo no prazo de 30 dias, acompanhado da proposta de honorários, ficando advertida de que eventuais atrasos serão levados em consideração na fixação do valor. Intime-se.</p>\
