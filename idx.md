...
| **Suite1 — Orquestrador enxuto TM** | `Script/suite1.user.js` (só Alvará + Argos + hcalc; painel próprio de 3 botões) |
| ↳ Armadilha do Suite1: pool de `@require` | Redeclarado no header (TM ignora `@require` aninhado). `alv.user.js` É exigido (único definidor de `window.PjeAlvara`) e deve vir DEPOIS de todo `Script/alvara/*`; `Script/hcalc.user.js` NÃO é exigido (boot replicado). |
