...
def navegar_para_pagamentos(driver, log=True):
    """
    Navega para a listagem de pagamentos abrindo uma nova aba.
    ...
    Returns:
        bool: True se conseguiu navegar com sucesso
    """
    try:
        if log:
            print('[PAGAMENTO] 🚀 Iniciando navegação para listagem de pagamentos...')
        
        # Armazenar URL atual
        url_atual = driver.current_url
        if log:
            print(f'[PAGAMENTO] URL atual: {url_atual}')
        
        # Extrair o ID do processo da URL atual
        # Formato atual: .../pjekz/processo/ID/detalhe
        match = re.search(r'/processo/(\d+)/detalhe', url_atual)
        if not match:
            if log:
                print('[PAGAMENTO][ERRO] Não foi possível extrair ID do processo da URL')
            return False
        
        id_processo = match.group(1)
        if log:
            print(f'[PAGAMENTO] ID do processo extraído: {id_processo}')
        
        # Construir URL da listagem de pagamentos
        # Baseado no padrão: https://pje.trt2.jus.br/pjekz/processo/pagamento/ID_PROCESSO
        base_url = url_atual.split('/pjekz/')[0]
        url_pagamentos = f"{base_url}/pjekz/processo/pagamento/{id_processo}"
        
        if log:
            print(f'[PAGAMENTO] URL de pagamentos: {url_pagamentos}')
        
        # Abrir nova aba e navegar para pagamentos
        ...
        if 'pagamento' in driver.current_url:
            if log:
                print('[PAGAMENTO] ✅ Navegação para pagamentos bem-sucedida')
            return True
        else:
            if log:
                print('[PAGAMENTO][ERRO] Falha na navegação - URL incorreta')
            return False
            
    except Exception as e:
        if log:
            print(f'[PAGAMENTO][ERRO] Erro na navegação: {e}')
        return False
