...
URL_NAVEGACAO = "https://pje.trt2.jus.br/pjekz/processo/8224076/detalhe"
...
def executar_testes(driver_pje):
    """Executa o fluxo AUD para processo em audiência (sem filtro)."""
    print("\n" + "=" * 80)
    print("TESTE AUD - MARCAR AUDIÊNCIA (SEM FILTRO)")
    print("=" * 80)

    try:
        from Fix.extracao import extrair_dados_processo
        from aud import acao_bucket_a

        extrair_dados_processo(driver_pje)
        ...
        try:
            ...
            if isinstance(num_list, list) and num_list:
                numero_processo = num_list[0]
            elif isinstance(num_list, str):
                numero_processo = num_list
            tipo = (dados.get('classeJudicial', {}) or {}).get('sigla', '').strip().upper()
        except Exception:
            pass
        if not numero_processo:
            print("❌ Não foi possível obter número do processo em dadosatuais.json")
            return False
        ...
        ok = acao_bucket_a(driver_pje, numero_processo, processo_info)
        if not ok:
            print("❌ Fluxo 100% sem audiência falhou")
            return False

        print("✅ Fluxo AUD concluído")
        return True
