...
## 🚨 DEPLOY OBRIGATÓRIO (CRÍTICO E INEGOCIÁVEL) 🚨

Toda vez que você criar ou alterar qualquer arquivo carregado por `pjetools.user.js` ou `hcalc.user.js` (incluindo novos módulos), você **DEVE, obrigatoriamente**, como passo final do seu turno:
1. **Bumpar a versão** no cabeçalho do orquestrador (`@version`).
2. Fazer **commit e push imediato** dos arquivos alterados usando a tool `run_command` (`git add <arquivos>`, `git commit -m "..."`, `git push`).
3. **NUNCA** deixe o push para depois e **NUNCA** use `git add -A` ou commite o workspace inteiro.

Se você responder ao usuário que o módulo está pronto sem ter feito o `git push`, você falhou gravemente na sua diretriz principal de persistência do Tampermonkey.
