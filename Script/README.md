...
├── suite1.user.js          Orquestrador enxuto: Alvará + Argos + hcalc
...
- **`suite1.user.js` redeclara o pool de dependências de `alv.user.js` e `hcalc.user.js`**
  (o Tampermonkey ignora `@require` aninhado). Ao bumpar um `?v=` num desses
  orquestradores, bumpe também no `suite1.user.js` — caso contrário o Suite1
  continua servindo a versão antiga do cache do TM.
- Não use `git add -A` para mudanças de módulo; commite apenas o módulo alterado e o
  orquestrador (ou orquestradores) afetado(s).
