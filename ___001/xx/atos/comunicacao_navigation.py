...
def abrir_minutas(driver, debug=False):
    ...
    try:
        ...
        if debug:
            print(f'[URL] URL atual: {current_url}')
        ...
        if debug:
            print(f'[URL] Abrindo URL de minutas: {url_minutas}')
        ...
        if debug:
            print(f'[URL][ERRO] Falha na navegação direta por URL: {url_error}')
            print('[URL] Fazendo fallback para navegação tradicional por cliques...')
