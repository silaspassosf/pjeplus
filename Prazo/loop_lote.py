...
def _verificar_quantidade_processos_paginacao(driver: Any) -> int:
    """
    ...
    try:
        ...
        if not total_elem:
            return -1
        texto = getattr(total_elem, 'text', '').strip()
...
def _ciclo1_aplicar_filtro_fases(driver: Any) -> Union[bool, str]:
    ...
    try:
        ...
        while time.perf_counter() - t1 < timeout_espera:
            try:
                celulas = espera.elementos(driver, seletor_celula_processo, teto=0.1)
                if len(celulas) > 0:
            ...
            try:
                vazios = espera.elementos(driver, xpath_vazio, teto=0.1)
                if any(getattr(el, 'is_displayed', lambda: True)() for el in vazios):
                    t_lista = time.perf_counter() - t1
            ...
            espera.pausa(driver, 0.15, "polling filtro fases")
...
def _ciclo1_marcar_todas(driver: Any) -> str:
    ...
    def _tentar_marcar():
        ...
        icone = espera.elemento(driver, "//i[contains(@class, 'marcar-todas')]", teto=5)
        if not icone:
            return False
        html = getattr(icone, 'get_attribute', lambda _: '')('outerHTML') or ''
...
def _ciclo1_abrir_suitcase(driver: Any) -> bool:
    ...
    def _tentar_abrir_suitcase():
        ...
        elemento = espera.elemento(driver, seletor, teto=8)
        if not elemento:
            return False
        ...
        if clicou:
            log_seletor_vencedor('CICLO1/SUITCASE', 'css', seletor)
...
def _ciclo1_aguardar_movimentacao_lote(driver: Any) -> bool:
...
def _ciclo1_movimentar_destino_providencias(driver: Any) -> bool:
    ...
    try:
        ...
        seta_dropdown = espera.elemento(driver, "div.mat-select-arrow-wrapper", teto=2)
        if not seta_dropdown:
            return False
        ...
        for xpath in [
            ...
            try:
                opcao_elemento = espera.elemento(driver, xpath, teto=0.5)
                if opcao_elemento:
                    break
            except Exception:
                pass
        ...
        btn = espera.elemento(driver, seletor_btn, teto=2)
        if not btn:
            return False
...
def _ciclo1_movimentar_destino(driver: Any, opcao_destino: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            seta_dropdown = espera.elemento(driver, seletor_dropdown, teto=2)
            if not seta_dropdown:
                return False
            safe_click_no_scroll(driver, seta_dropdown)
            log_seletor_vencedor('CICLO1/DESTINO_DROPDOWN', 'css', seletor_dropdown)
        except Exception as e:
        ...
        try:
            opcao_elemento = espera.elemento(driver, opcao_xpath, teto=2)
            if not opcao_elemento:
                return False
            log_seletor_vencedor('CICLO1/DESTINO_OPCAO', 'xpath', opcao_xpath)
        except Exception as e:
        ...
        try:
            ...
            btn_movimentar = espera.elemento(driver, seletor_movimentar, teto=2)
            if not btn_movimentar:
                return False
            safe_click_no_scroll(driver, btn_movimentar)
            log_seletor_vencedor('CICLO1/DESTINO_BOTAO_MOVIMENTAR', 'css', seletor_movimentar)
        except Exception as e:
...
def _ciclo1_retornar_lista(driver: Any) -> None:
    ...
    try:
        ...
        _executar_js(driver, "window.history.back();")
        try:
            aguardar_renderizacao_nativa(driver, 'span.total-registros', timeout=2)
        except Exception:
            aguardar_renderizacao_nativa(driver, timeout=2)  # fallback: DOM ready
        # Garantir fechamento de modais se sobrarem
        _executar_js(driver, "document.querySelectorAll('.cdk-overlay-backdrop').forEach(e => e.click())")
    except Exception as e:
...
def _ciclo2_aplicar_filtros(driver: Any) -> bool:
...
def _ciclo2_processar_livres(driver: Any, client: Optional['PjeApiClient'] = None) -> int:
    ...
    try:
        ...
        if processos_com_gigs_api:
            selecionados_livres = _executar_js(driver, SCRIPT_SELECAO_LIVRES_API, processos_com_gigs_api)
            _label = 'SCRIPT_SELECAO_LIVRES_API'
        else:
            selecionados_livres = _executar_js(driver, SCRIPT_SELECAO_LIVRES)
...
def _ciclo2_selecionar_nao_livres(driver: Any, max_processos: int = 20) -> Tuple[int, bool]:
    """Seleciona processos não-livres via JavaScript."""
    try:
        # Desselecionar todos primeiro — aguardar Angular estabilizar
