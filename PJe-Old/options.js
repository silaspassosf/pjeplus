...
async function checarVariaveis(restaurar=false) {
	return new Promise(async resolve => {
		...
		opcoes.perfilUsuario = typeof(opcoes.perfilUsuario) == "undefined" ? "" : opcoes.perfilUsuario;
		...
		opcoes.gigsOcultarCabecalho = typeof(opcoes.gigsOcultarCabecalho) == "undefined" ? 0 : opcoes.gigsOcultarCabecalho;
		...
		opcoes.aaVariados = typeof(opcoes.aaVariados) == "undefined" ? [{id:"Atualizar Pagina",nm_botao:"Atualizar Pagina",descricao:"AA para ATUALIZAR a janela DETALHES DO PROCESSO",temporizador:"1",ativar:true},{id:"Fechar Pagina",nm_botao:"Fechar Pagina",descricao:"AA para fechar a janela DETALHES DO PROCESSO",temporizador:"3",ativar:true},{id:"Apreciar Peticoes",nm_botao:"Apreciar Peticoes",descricao:"AA para apreciar petições na janela DETALHES DO PROCESSO",temporizador:"1",ativar:true},{id:"Atalho F2",nm_botao:"Atalho F2",descricao:"Funcionalidade do Menu KAIZEN: Permite ativar/desativar a criação de uma área na tela para acionar a tecla 'F2' ao repousar o mouse em cima pelo tempo ajustado no 'temporizador'.",temporizador:"2",ativar:true},{id:"Atalho F3",nm_botao:"Atalho F3",descricao:"Funcionalidade do Menu KAIZEN: Permite ativar/desativar a criação de uma área na tela para acionar a tecla 'F3' ao repousar o mouse em cima pelo tempo ajustado no 'temporizador'.",temporizador:"2",ativar:true},{id:"SISBAJUD F2",nm_botao:"SISBAJUD F2",descricao:"Funcionalidade SISBAJUD: ativa automaticamente uma minuta de bloqueio de valores sobre o polo passivo no convênio SISBAJUD. A janela do convênio precisa estar aberta e visível.",temporizador:"2",ativar:true}] : opcoes.aaVariados;
		
		if (opcoes.aaVariados[4]) {
			if (opcoes.aaVariados[4].id == 'SISBAJUD F2') {
				opcoes.aaVariados.splice(4, 0, {id:"Atalho F3",nm_botao:"Atalho F3",descricao:"Funcionalidade do Menu KAIZEN: Permite ativar/desativar a criação de uma área na tela para acionar a tecla 'F3' ao repousar o mouse em cima pelo tempo ajustado no 'temporizador'.",temporizador:"2",ativar:true});
			}
		} else { 
			opcoes.aaVariados.push({id:"Atalho F3",nm_botao:"Atalho F3",descricao:"Funcionalidade do Menu KAIZEN: Permite ativar/desativar a criação de uma área na tela para acionar a tecla 'F3' ao repousar o mouse em cima pelo tempo ajustado no 'temporizador'.",temporizador:"2",ativar:true});
			opcoes.aaVariados.push({id:"SISBAJUD F2",nm_botao:"SISBAJUD F2",descricao:"Funcionalidade SISBAJUD: ativa automaticamente uma minuta de bloqueio de valores sobre o polo passivo no convênio SISBAJUD. A janela do convênio precisa estar aberta e visível.",temporizador:"2",ativar:true});
		}
		...
		opcoes.modulo9 = typeof(opcoes.modulo9) == "undefined" ? {sisbajud:true,renajud:true,cnib:true,serasajud:true,ccs:[true,false,false,false,false,5],crcjud:true,onr:false,gprec:true,ajjt:true,siscondj:true,garimpo:true,sif:true,pjecalc:true,prevjud:true,protestojud:true,sniper:true,censec:true,celesc:true,casan:true,sigef:true,infoseg:true,ecarta:true} : opcoes.modulo9;
		opcoes.modulo9.ccs = Array.isArray(opcoes.modulo9.ccs) ? opcoes.modulo9.ccs : [opcoes.modulo9.ccs,false,false,false,false,5];
		...
		opcoes.anexadoDoctoEmSigilo = typeof(opcoes.anexadoDoctoEmSigilo) == "undefined" ? false : opcoes.anexadoDoctoEmSigilo;
...
async function mostrarOpcoes(restaurar=false) {
	return new Promise(async resolve => {
		...
		if (opcoes.modoLGPD) {
			...
			document.getElementById("modoLGPD").parentElement.setAttribute('data-tooltip','Desativar modo LGPD');		
		} else {
			document.getElementById("modoLGPD").className = 'icone lgpd-off t20';
			document.getElementById("modoLGPD").parentElement.setAttribute('data-tooltip','Ativar modo LGPD');
		}
		...
		document.getElementById("perfilUsuarioVar").innerText = opcoes.perfilUsuario != null ? opcoes.perfilUsuario : "nenhum";
		...
		document.getElementById("perfilUsuario").addEventListener('click', perfilUsuario);
...
async function salvarOpcoes() {
	return new Promise(async resolve => {
		...
		for (const [pos, item] of document.querySelectorAll('tr[id="lista_modulo10_item"]').entries()) {
			...
			if (h || u) {
				listaModulo10_temp.push({horario:h,url:u});
			}
		}
		...
		await browser.storage.local.set({
			...
			perfilUsuario: document.getElementById("perfilUsuarioVar").innerText,
...
async function filtro_Magistrado() {
	const { value: result } = await Swal.fire({
		...
		html:
			...
			'<div id="escolherProcessoFinal"><span>Processo final:</span><br><br>' +
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro0" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">0</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' + 
			'<input type="checkbox" id="modulo8_Filtro1" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">1</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro2" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">2</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro3" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">3</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro4" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">4</span>' +
			'<span class="checkmark"></span>' +
			'</label><br>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro5" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">5</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro6" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">6</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro7" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">7</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro8" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">8</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<label class="container">' +
			'<input type="checkbox" id="modulo8_Filtro9" checked><span style="position: relative; padding-right: 15px; top: 2px; margin-left: -10px; font-size: 24px;">9</span>' +
			'<span class="checkmark"></span>' +
			'</label>' + 
			'<br><br><div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 1em; cursor: pointer;">' +
			'<span id="maisPje_parEimpar" name="par" class="swal2-label spanToogle">Par</span>' +
...
async function modulo9(convenio) {
	switch (convenio) {			
		case 'sisbajud':
			...
			//*****ajusta a teimosinha para a configuração de 30 e 60 dias
			...
			if (teimosinha.includes('60')) {
				...
				teimosinha = '30';
			} else if (teimosinha.toLowerCase().includes('sim')) {
				teimosinha = '60';
			} else {
				teimosinha = 'nao'
			}			
			...
			let { value: resultsisbajud } = await Swal.fire({
				...
				html:
					...
					'<input id="swal-input8" class="swal2-input"" value="' + opcoes.sisbajud.banco_preferido + '">' +
					'<span style="font-weight: bold;"> Agência preferida (em caso de bloqueio): </span>' + 
					'<input id="swal-input9" class="swal2-input"" value="' + opcoes.sisbajud.agencia_preferida + '">' +
		...
		case 'gprec':
			let { value: resultgprec } = await Swal.fire({
				...
				html:
					...
					'<input type="checkbox" id="swal2-checkbox"' + (opcoes.modulo9.gprec ? ' checked ' : '') + '>' +
					'<span class="swal2-label"> Ativar</span>',
				focusConfirm: false,
				confirmButtonText: 'Salvar',
				width: 800,
			...
			if (resultgprec) {
				opcoes.modulo9.gprec = resultgprec[0];
				await guardarModulo9();
				document.getElementById("convenio_gprec").style.backgroundColor = (opcoes.modulo9.gprec) ? '#2196F3' : '#b0b0b0';
			}
			break
		...
		case 'siscondj':
			let { value: resultsiscondj } = await Swal.fire({
				...
				html:
					...
					'<input type="checkbox" id="swal2-checkbox"' + (opcoes.modulo9.siscondj ? ' checked ' : '') + '>' +
					'<span class="swal2-label"> Ativar</span>',
				focusConfirm: false,
				confirmButtonText: 'Salvar',
				width: 800,
			...
			if (resultsiscondj) {
				opcoes.modulo9.siscondj = resultsiscondj[0];
				await guardarModulo9();
				document.getElementById("convenio_siscondj").style.backgroundColor = (opcoes.modulo9.siscondj) ? '#2196F3' : '#b0b0b0';
			}
			break			
		...
		case 'sif':
			let { value: resultsif } = await Swal.fire({
				...
				html:
					...
					'<input type="checkbox" id="swal2-checkbox"' + (opcoes.modulo9.sif ? ' checked ' : '') + '>' +
					'<span class="swal2-label"> Ativar</span><br><br>' + 
					...
					'<input id="swal-input1" class="swal2-input" style="background-color: white;" value="' + opcoes.tempAuto + '"></div>',
				focusConfirm: false,
				confirmButtonText: 'Salvar',
				width: 800,
				preConfirm: () => {
			...
			if (resultsif) {
				opcoes.modulo9.sif = resultsif[0];
				opcoes.tempAuto = resultsif[1];
				await guardarModulo9();
				document.getElementById("convenio_sif").style.backgroundColor = (opcoes.modulo9.sif) ? '#2196F3' : '#b0b0b0';
			}
			break
		...
		case 'ecarta':
			let { value: resultecarta } = await Swal.fire({
				...
				html:
					...
					'<input type="checkbox" id="swal2-checkbox"' + (opcoes.modulo9.ecarta ? ' checked ' : '') + '>' +
					'<span class="swal2-label"> Ativar</span>',
				focusConfirm: false,
				confirmButtonText: 'Salvar',
				width: 800,
			...
			if (resultecarta) {
				opcoes.modulo9.ecarta = resultecarta[0];
				await guardarModulo9();
				document.getElementById("convenio_ecarta").style.backgroundColor = (opcoes.modulo9.ecarta) ? '#2196F3' : '#b0b0b0';
			}
			break
...
async function montar_modulo10() {
	...
	for (const [pos, item] of listaModulo10_temp.entries()) {
		...
		await inserirlinha(h,e);
	}
	
	await inserirlinha();
	
	async function inserirlinha(horario="",url="",foco=false) {
		let tr = document.createElement('tr');
...
async function perfilUsuario() {
	Swal.fire({
		title: 'Perfil de usuário favorito',
		html: 'No PJe, escolha seu perfil favorito clicando na caixa de seleção ao lado do nome.<br><br><div class="tutorial5" style="width: 261px;height: 289px;border-radius: .3125em;box-shadow: 0 0 0.3em #bbb;margin: auto;"></div><br><br>Na próxima vez que você entrar no PJe, este perfil será selecionado automaticamente.',
	});
}
...
async function importarAA() {
	const { value: result } = await Swal.fire({
		title: '    ',
		html:
		'<span style="font-weight: bold;"> Cole aqui as configurações da AÇÃO AUTOMATIZADA:</span><br>' + 
		'<textarea id="swal-input-importarAA" class="swal2-input" style="height: 500px;font-family: inherit;line-height: 1.5em;margin:0;"></textarea>',
		confirmButtonText: 'Importar',
		focusConfirm: false,
		preConfirm: () => {
			
			let configuracoes = document.getElementById('swal-input-importarAA').value;
			configuracoes = configuracoes.split(";");
			
			console.log('Tamanho: ' + configuracoes.length)
			...
			if (result[0] != "") {
				...
				switch (this.id) {
					...
					case 'aa_imp_despacho':
						...
						for (var i = 0; i < linha.length; i++) {		
							let temp = JSON.parse(linha[i]);
							let tempAssinar = (!temp.assinar) ? 'não' : temp.assinar;
...
async function montarBotoesaaVariados() {
	if (typeof(aaVariados_temp) != "undefined") {
		...
		for (const [pos, item] of aaVariados_temp.entries()) {	
			let aa = item;
			criaBotao_aaVariados(pos, aa.nm_botao, aa.descricao, aa.temporizador, aa.ativar);
...
function criaBotao_aaVariados(id, nm_botao, descricao, temporizador, ativar=false) {
	let el = document.getElementById("botoes_variados");
...
function listaAcoesAutomatizadas(vinculo) {
	...
	teste = [].map.call(aaItemLancarMovimentos,function(item) {
		lista += "<option style='background-color: #d3d3d35e;' value='LançarMovimento|" + item[1] + "'";
		lista += ('LançarMovimento|' + item[1] == vinculo) ? " selected> LançarMovimento|" + item[1] : "> LançarMovimento|" + item[1];
		lista += " </option>";
	});
...
async function modalEditorEditar(elemento_pai, tipo, id) {
	if (tipo == "anexar") {
		...
		function texto_html() {
			...
			switch (aaAutogigs_temp[id].tipo) {					
				...
				default:					
					...
					'<span style="font-weight: bold;"> Responsável pelo processo </span>' + 
					'<input id="swal-input6" class="swal2-input" value="' + aaAutogigs_temp[id].responsavel_processo + '">' +
					'<span style="font-weight: bold;"> Observação <br><i style="font-size: 0.8em;font-weight: normal;font-style: normal;text-align: left;color: darkcyan;"><br>aceita a expressão <b>"perguntar"</b> e <b>"corrigir data"</b> que interrompe a ação para o usuário informar o que quer preencher no campo</i></span>' + 
					'<textarea id="swal-input7" class="swal2-textarea">' + aaAutogigs_temp[id].observacao + '</textarea>' +
					'<span style="font-weight: bold;"> Salvar </span>' + 
					'<br><select id="swal-input8" class="swal2-select" style="background-color: white;width: 100%;border: 1px solid #d9d9d9;border-radius: .1875em;box-shadow: inset 0 1px 1px rgba(0,0,0,.06);height: 2.625em;padding: 0 .75em;">' + 
						'<option value="nao"' + (aaAutogigs_temp[id].salvar.toLowerCase().includes('nao') ? 'selected' : '')  + '> Não </option>' + 
		...
		function texto_html() {
			...
			switch (aaVariados_temp[id].nm_botao) {					
				case 'Atalho F2':
					...
					'<br><select id="swal-input2" class="swal2-select" style="background-color: white;width: 100%;border: 1px solid #d9d9d9;border-radius: .1875em;box-shadow: inset 0 1px 1px rgba(0,0,0,.06);height: 2.625em;padding: 0 .75em;">' + 
				...
				default:					
					...
					'<br><select disabled id="swal-input2" class="swal2-select" style="display:none;background-color: white;width: 100%;border: 1px solid #d9d9d9;border-radius: .1875em;box-shadow: inset 0 1px 1px rgba(0,0,0,.06);height: 2.625em;padding: 0 .75em;">' + 
		...
		const { value: result } = await Swal.fire({
			...
			focusConfirm: false,
			preConfirm: () => {
				let atv = (document.getElementById('swal-input2').value == 'sim') ? true : false;
				return [
					document.getElementById('swal-input1').value,
					atv
				]
	...
	if (event.target.className.includes("icone question-circle")) {
		...
		switch (event.target.id) {
			...
			case 'atalhos_1':
				...
				break
			case 'atalhos_2':
				msg = 'Aqui é possível atribuir um atalho ao menu do maisPJe, acessível ao clicar no ícone da extensão na barra de endereços.<br><br>Para isso você deve escrever em cada linha o nome do atalho, dois pontos (:), o endereço do atalho.<br><br>Por exemplo:<br>&nbsp;&nbsp;&nbsp; Intranet:http://intranet.trt12.jus.br/caslogin';
				break
			case 'config_1':
