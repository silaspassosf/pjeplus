...
2. **Prefixo de prompt**: Iniciar comandos com `[REF: refat.md + RULES]` 
...
- Quando for necessario usar `ref` para comparar funcoes, procurar no legado ate identificar o trecho exato para comparacao. E garantido que o trecho existe no legado, entao nao pode desistir da busca. Nao se deve copiar funcoes de `ref` para o projeto refatorado; ajustar apenas as funcoes prontas do projeto refatorado usando o INDEX.md como guia. O INDEX.md e o mapa do projeto refatorado; `ref` e apenas backup do codigo legado.
