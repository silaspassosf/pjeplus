...
// Portado de Script/modules/lista/lista.check.js
// Sem GM_*, sem @require externos — expõe tudo em window.*

const log = window.PJeLogger || {
    check:   (...a) => console.log('[PJeT::CHECK]', ...a),
    api:     (...a) => console.log('[PJeT::API]', ...a),
    dom:     (...a) => console.log('[PJeT::DOM]', ...a),
    warn:    (c, ...a) => console.warn(`[PJeT::${c}]`, ...a),
    error:   (c, m, e, extra) => console.error(`[PJeT::${c} ERRO] ${m}:`, e, extra),
...
window.lerTimelineCompleta = async function () {
    ...
    if (state.docs && (agora - state.readAt) < CACHE_TTL) {
        log.check(`Cache da timeline utilizado: ${state.docs.length} documentos (lido há ${Math.round((agora - state.readAt)/1000)}s).`);
        return state.docs;
    }

    const idProcesso = _pjeTlIdProcesso();
    if (!idProcesso) {
        log.error('CHECK', 'Não foi possível detectar o ID do processo pela URL atual', null, { url: window.location.href });
        showToast('ID do processo não encontrado na URL', '#dc3545', 3500);
        return [];
    }

    log.api(`Consultando timeline do processo ID ${idProcesso} via API...`);
    let itensApi = [];
    ...
    try {
        ...
        const headers = _pjeTlHeaders();
        const resp = await fetch(url, { method: 'GET', credentials: 'include', headers });
        const tempo = Math.round(performance.now() - t0);

        if (!resp.ok) {
            log.error('API', `HTTP ${resp.status} (${resp.statusText || 'Erro'}) ao buscar timeline em ${tempo}ms`, null, {
                status: resp.status,
                url,
                xsrfPresente: !!headers['X-XSRF-TOKEN']
            });
            showToast(`Falha ao obter timeline (HTTP ${resp.status})`, '#dc3545', 4000);
            return [];
        }

        const raw = await resp.text();
        itensApi = JSON.parse(raw);
        log.success('API', `Timeline retornou ${itensApi.length} itens da API em ${tempo}ms.`);
    } catch (e) {
        const tempo = Math.round(performance.now() - t0);
        log.error('API', `Falha de rede/parse ao buscar timeline (${tempo}ms)`, e);
        showToast('Erro de comunicação ao carregar timeline', '#dc3545', 4000);
        return [];
    }

    if (!itensApi.length) {
        log.warn('API', 'A API retornou lista vazia de itens para o processo.');
        return [];
    }
    ...
    for (const item of itensApi) {
        ...
        for (const anexo of anexosApi) {
            ...
            contadores[tipoAnexo] = (contadores[tipoAnexo] || 0) + 1;
    ...
    log.check(`Classificação concluída: ${documentos.length} documento(s) relevante(s).`, contadores);
...
async function onCheckRowClick(doc) {
    ...
    if (doc.isAnexo && (doc.tipo === 'Serasa' || doc.tipo === 'CNIB') && doc.parentId) {
        log.check(`Navegando para anexo ${doc.tipo} (${doc.id}) vinculado ao pai ${doc.parentId}`);
        const parentItem = encontrarElementoPorUid(doc.parentId);
        if (!parentItem) {
            log.warn('CHECK', `Elemento pai (${doc.parentId}) da certidão não encontrado no DOM para anexo: ${doc.id}`);
            return;
        }
        ...
        alvo = alvo || anexoLinks[0];
        if (!alvo) {
            log.warn('CHECK', `Link do anexo não encontrado no DOM: ${doc.id}`);
            return;
        }
        ...
        const anexoHref = alvo.getAttribute('href');
        if (anexoHref) {
            log.check(`Abrindo anexo em nova aba: ${anexoHref}`);
    ...
    const elem = resolverElemento(doc) || encontrarElementoPorUid(doc.id);
    if (!elem) {
        log.warn('CHECK', `Elemento não encontrado no DOM para doc ID: ${doc.id}`);
        return;
    }
...
window.executarCheck = async function () {
    log.check('Executando módulo Check...');
    try {
        const docs = await lerTimelineCompleta();
        if (!docs || !docs.length) {
            log.warn('CHECK', 'Nenhum documento retornado da timeline.');
            showToast('Nenhum documento encontrado na timeline', '#f57f17', 3500);
            return;
        }

        const filtrados = filtrarDocs(docs);
        log.check(`Filtragem: ${filtrados.length} doc(s) de ${docs.length} mantidos.`);
        const saida = construirOrdem(filtrados);
        log.check(`Construção de ordem: ${saida.length} item(ns) ordenados para exibição.`);

        renderTabela('listaDocsExecucaoSimples', '📋 Relatório de Medidas', '#007bff',
            saida, onCheckRowClick);
        log.success('CHECK', 'Painel de Relatório de Medidas criado no DOM.');

        // Execução automática após gerar a tabela
        setTimeout(() => {
            window.autoSelecionarPesquisaCheck().catch(e => {
                log.error('CHECK', 'Erro ao executar autoSelecionarPesquisaCheck', e);
            });
        }, 500);
        ...
        if (panel) {
            ...
            if (hdr) {
                ...
                btnAuto.onclick = async (e) => {
                    ...
                    catch (err) { log.error('CHECK', 'Erro no auto-check:', err); }
                };
                ...
                btn.onclick = async (e) => {
                    e.stopPropagation();
                    e.preventDefault();
                    log.check('Botão [Conferir alvarás] acionado via header.');
                    try {
                        if (typeof window.executarPgto === 'function') {
                            setTimeout(() => {
                                window.executarPgto().catch(err => {
                                    log.error('PGTO', 'Erro ao executar conferir alvarás', err);
                                });
                            }, 100);
                        } else {
                            log.warn('PGTO', 'window.executarPgto não encontrado');
                        }
                    } catch (e) {
                        log.error('PGTO', 'Erro síncrono ao disparar conferir alvarás', e);
                    }
                };
        ...
        log.error('CHECK', 'Falha ao executar Check', err);
    }
}
...
window.executarBaixarAutomatico = async function (saida) {
    ...
    log.check(`Baixar Automático: ${serasaCnib.length} item(ns) Serasa/CNIB encontrados.`);
    if (!serasaCnib.length) {
        showToast('Nenhum Serasa/CNIB para baixar', '#6c757d', 3000);
        return;
    }

    for (let i = 0; i < serasaCnib.length; i++) {
        const item = serasaCnib[i];
        log.check(`Baixando [${i + 1}/${serasaCnib.length}]: ${item.tipo} (${item.id})`);
        try {
            ...
            log.error('CHECK', `Erro ao baixar item ${item.id}`, e);
        }
    }

    invalidarCacheTimeline();
    log.success('CHECK', 'Download automático finalizado.');
}
