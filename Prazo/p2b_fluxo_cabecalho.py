...
def _processar_checar_cabecalho(driver: WebDriver) -> None:
    """
    Helper: Processa checagem de cabeçalho para "descumprimento".
    Sempre executa PESQLIQ diretamente, ignorando cor do cabeçalho,
    pois não é possível mover para execução antes.
    """
    # Lazy load modules
    m = _lazy_import()
    ato_pesquisas = m['ato_pesquisas']
    ...
    try:
        ...
        # 2. Executar ato_pesqliq diretamente
        ...
        logger.error(f'[FLUXO_PZ] Erro no processamento direto de descumprimento (PESQLIQ): {cabecalho_error}')
