...
const LSSUGESTOES = 'audicheck_local_sugestoes';
...
const dbSaveProcessos = async (procs: any) => {
  const rows = Object.entries(procs).map(([id, p]: any) => ({
    ...
    hora: p.hora,
    partes: p.partes,
    // partesApi NUNCA persiste em DB (local ou Supabase) — exclusiva do estado em memória
    tipo: p.tipo,
    tipo_aud: p.tipoAud || "",
...
const dbEncerrar = async (id: string) => {
  ...
  lsSet(LS_CHECKINS, checkins.filter((c: any) => c.processo_id !== id));
  const procs: any[] = lsGet(LS_PROCESSOS, []);
  lsSet(
    LS_PROCESSOS,
    procs.map((p: any) => (p.id === id ? { ...p, encerrada: true } : p))
  );
};
...
const dbSaveSugestoes = async (processoId: string, partesApi: any[]) => {
  const todas: any[] = lsGet(LSSUGESTOES, []);
  const filtradas = todas.filter((s: any) => s.processo_id !== processoId);
  const novas = partesApi
    .filter((p: any) => p.nomeAdvogado?.trim())
    .map((p: any) => ({
      processo_id: processoId,
      polo: p.polo,
      nomeAdvogado: p.nomeAdvogado.trim(),
      oab: ((p.uf || 'SP') + (p.oab || '')).toUpperCase().replace(/[^A-Z0-9]/g, ''),
      nome: p.nome?.trim() ?? '',
    }));
  lsSet(LSSUGESTOES, [...filtradas, ...novas]);
};

const dbGetSugestoes = async (processoId: string): Promise<any[]> => {
  const todas: any[] = lsGet(LSSUGESTOES, []);
  return todas.filter((s: any) => s.processo_id === processoId);
};

// ── CONSTANTES ──
const RAIO_METROS = 1000;
const DEV_MODE = true; // ← desativa geolocalização em modo local
  ...
  const gerar = () => {
    ...
    const carregar = () => {
      ...
      Promise.all(divs).then((qrs: any) => {
        ...
        const data = lista[0]?.[1].data || new Date().toLocaleDateString('pt-BR');
...
// ── MODAL IMPORTAR VIA API PJe ──
function MImportPje({ onImportar, onFechar, onFallbackManual }: {
  onImportar: (procs: any) => void;
  onFechar: () => void;
  onFallbackManual: () => void;
}) {
  // ── lê bridge do localStorage ──
  const lerBridge = () => {
    try {
      const raw = localStorage.getItem('audicheck_pauta_bridge');
      const ts = localStorage.getItem('audicheck_pauta_bridge_ts');
      const data = localStorage.getItem('audicheck_pauta_bridge_data');
      if (!raw || !ts) return null;
      const idadeMin = (Date.now() - parseInt(ts)) / 60000;
      if (idadeMin > 120) return null; // expira em 2h
      return { procs: JSON.parse(raw) as { [k: string]: any }, idadeMin: Math.round(idadeMin), data };
    } catch { return null; }
  };

  const bridge = lerBridge();
  const total = bridge ? Object.keys(bridge.procs).length : 0;

  const confirmar = () => {
    if (!bridge) return;
    onImportar(bridge.procs);
    localStorage.removeItem('audicheck_pauta_bridge');
    localStorage.removeItem('audicheck_pauta_bridge_ts');
    localStorage.removeItem('audicheck_pauta_bridge_data');
    onFechar();
  };

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 16, fontFamily: 'Segoe UI,sans-serif', animation: 'fadeIn .2s ease' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 26, maxWidth: 480, width: '100%', boxShadow: '0 16px 48px rgba(0,0,0,.3)', animation: 'fadeInUp .25s ease' }}>
        <h3 style={{ margin: '0 0 4px', color: '#1a3a6b' }}>📋 Importar Pauta via PJe</h3>

        {/* ── sem bridge ── */}
        {!bridge && (
          <div>
            <div style={{ background: '#f0f4ff', borderRadius: 10, padding: '16px 18px', fontSize: 13, color: '#1a3a6b', marginBottom: 18, lineHeight: 1.7 }}>
              <strong>Como usar:</strong><br />
              1. Abra o PJe e faça login normalmente.<br />
              2. Clique no favorito <strong>"AudiCheck Bridge"</strong> na barra do navegador.<br />
              3. Aguarde a confirmação <em>"audiências enviadas"</em> e volte aqui.<br />
              4. Clique em <strong>Verificar</strong> para importar.
            </div>
            <div style={{ background: '#fff3cd', border: '1px solid #ffe082', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#7b5e00', marginBottom: 14 }}>
              Nenhuma pauta recebida do PJe ainda. Execute o bookmarklet no PJe e volte.
            </div>
          </div>
        )}
        ...
        {bridge && (
          <div>
            <div style={{ background: '#e8f5e9', border: '1.5px solid #4caf50', borderRadius: 10, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: '#2e7d32' }}>
              ✅ <strong>{total} audiência{total !== 1 ? 's' : ''}</strong> recebida{total !== 1 ? 's' : ''} do PJe há <strong>{bridge.idadeMin} min</strong> · data <strong>{bridge.data}</strong>
            </div>
            <ul style={{ margin: '0 0 16px', paddingLeft: 20, fontSize: 12, color: '#333', lineHeight: 1.8 }}>
              {Object.entries(bridge.procs)
                .sort(([, a]: any, [, b]: any) => a.hora.localeCompare(b.hora))
                .map(([num, info]: any) => (
                  <li key={num}>
                    <strong>{info.hora}</strong>{' '}
                    <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#888' }}>{num}</span>
                    {info.tipo === 'videoconferência' && (
                      <span style={{ marginLeft: 6, background: '#e3f2fd', color: '#1565c0', borderRadius: 4, padding: '1px 6px', fontSize: 10, fontWeight: 700 }}>VIRTUAL</span>
                    )}
                    <br />
                    <span style={{ color: '#555' }}>{info.partes?.slice(0, 65)}{(info.partes?.length ?? 0) > 65 ? '…' : ''}</span>
                  </li>
                ))}
            </ul>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as any }}>
          <button onClick={onFechar} style={{ flex: 1, background: '#eee', color: '#555', border: 'none', borderRadius: 9, padding: 11, fontSize: 13, cursor: 'pointer' }}>
            Cancelar
          </button>
          <button onClick={onFallbackManual} style={{ flex: 1, background: '#fff', color: '#1a3a6b', border: '1.5px solid #1a3a6b', borderRadius: 9, padding: 11, fontSize: 12, cursor: 'pointer' }}>
            PDF manual
          </button>
          {!bridge && (
            <button onClick={() => { /* força releitura fechando e reabrindo */ onFechar(); }} style={{ flex: 2, background: '#1a3a6b', color: '#fff', border: 'none', borderRadius: 9, padding: 11, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              🔄 Verificar novamente
            </button>
          )}
          {bridge && (
            <button onClick={confirmar} style={{ flex: 2, background: '#27ae60', color: '#fff', border: 'none', borderRadius: 9, padding: 11, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
              ✓ Importar {total} audiência{total !== 1 ? 's' : ''}
            </button>
          )}
        </div>
      </div>
...
function PSec({ parts, procs, setProcs, setParts, onVoltar, onLimpar, onEncerrar }: any) {
  ...
  const [mImportPje, setMImportPje] = useState(false);
  ...
  const impApi = async (procsApi: any) => {
    const q = Object.keys(procsApi).length;
    if (!q) { showToast('Nenhuma audiência encontrada.', 'erro'); return; }
    // O campo partesApi já vem no objeto e deve ser preservado sem transformação.
    const u = { ...procs, ...procsApi };
    await Promise.all(
      Object.entries(procsApi).map(([num, info]: any) =>
        info.partesApi?.length ? dbSaveSugestoes(num, info.partesApi) : Promise.resolve()
      )
    );
    await setProcs(u);
    showToast(`${q} audiência${q !== 1 ? 's' : ''} importada${q !== 1 ? 's' : ''} via PJe!`);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#eef2f7", fontFamily: "'Segoe UI',sans-serif" }}>
      ...
      {mImportPje && (
        <MImportPje
          onImportar={impApi}
          onFechar={() => setMImportPje(false)}
          onFallbackManual={() => { setMImportPje(false); setMImport(true); }}
        />
      )}
      ...
      >
        ...
        <button
          onClick={() => setMImportPje(true)}
          style={{ background: '#8e44ad', border: 'none', color: '#fff', borderRadius: 7, padding: '7px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}
        >
          📋 Aud via PJe
        </button>
        <button
...
function Login({ onLogin, onVoltar, titulo = "Secretário(a)", cor = "#1a3a6b" }: any) {
  ...
  const [bridgeInfo, setBridgeInfo] = useState<{
    total: number; data: string; procs: any;
  } | null>(() => {
    try {
      const raw = localStorage.getItem('audicheck_pauta_bridge');
      const ts = localStorage.getItem('audicheck_pauta_bridge_ts');
      const dt = localStorage.getItem('audicheck_pauta_bridge_data');
      if (!raw || !ts) return null;
      if ((Date.now() - parseInt(ts)) / 60000 > 120) return null;
      const procs = JSON.parse(raw);
      return { total: Object.keys(procs).length, data: dt ?? '', procs };
    } catch { return null; }
  });
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          background: bridgeInfo ? '#e8f5e9' : '#f8faff',
          border: `1.5px solid ${bridgeInfo ? '#4caf50' : '#e0e8f5'}`,
          transition: 'all .3s ease',
        }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: bridgeInfo ? '#2e7d32' : '#1a3a6b', marginBottom: 6 }}>
            📋 Dados do PJe
          </div>

          {bridgeInfo ? (
            /* ── dados prontos: mostra resumo + botão limpar ── */
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: '#2e7d32' }}>
                ✅ <strong>{bridgeInfo.total}</strong>{' '}
                audiência{bridgeInfo.total !== 1 ? 's' : ''} · {bridgeInfo.data}
              </span>
              <button
                type="button"
                onClick={() => { setBridgeInfo(null); limparBridge(); }}
                title="Descartar dados do PJe"
                style={{ background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', fontSize: 16, lineHeight: 1, padding: '0 2px' }}
              >×</button>
            </div>
          ) : (
            /* ── sem dados: textarea sempre disponível para colar ── */
            <textarea
              placeholder="Cole aqui após executar o favorito no PJe…"
              onChange={e => {
                ...
                try {
                  ...
                  if (!total) return;
                  const dt = (Object.values(procs)[0] as any)?.data ?? '';
                  setBridgeInfo({ total, data: dt, procs });
                  localStorage.setItem('audicheck_pauta_bridge', raw);
                  localStorage.setItem('audicheck_pauta_bridge_ts', String(Date.now()));
                  localStorage.setItem('audicheck_pauta_bridge_data', dt);
                  e.target.value = '';
                } catch { /* JSON inválido — aguarda mais colagem */ }
              }}
              style={{
                width: '100%', height: 60, padding: '8px 10px',
                border: '1.5px solid #e0e8f5', borderRadius: 8,
                fontSize: 11, fontFamily: 'monospace', color: '#555',
                boxSizing: 'border-box' as any, resize: 'none',
...
function GPS({ info, onOk, onVoltar, jaValidou }: any) {
  ...
  const ver = () => {
    ...
    if (DEV_MODE) {
      handleSuccess({ coords: { latitude: FORUM.lat, longitude: FORUM.lng } });
    } else {
...
// ── SUGESTÃO DE ADVOGADO VIA PJe ──
function SugestaoAdv({ partesApi, polo, nomeReclamada, onSelecionar, selecionado }: {
  partesApi: any[];
  polo: 'ATIVO' | 'PASSIVO';
  nomeReclamada?: string;
  onSelecionar: (nome: string, oab: string, empresa?: string) => void;
  selecionado: boolean;
}) {
  const candidatos = partesApi.filter((p: any) => {
    if (p.polo !== polo) return false;
    if (!p.nomeAdvogado?.trim()) return false;
    if (polo === 'PASSIVO' && nomeReclamada) {
      return p.nome.trim().toLowerCase().includes(nomeReclamada.trim().toLowerCase().slice(0, 20));
    }
    return true;
  });
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: polo === 'ATIVO' ? '#1a7a3a' : '#92400e', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
        {polo === 'ATIVO' ? '📋 Advogado do Reclamante (PJe)' : `📋 Advogado da Reclamada (PJe)`}
      </div>
      {candidatos.map((p: any, i: number) => {
        const nome = p.nomeAdvogado.trim();
        const oab = fOAB((p.uf || 'SP') + (p.oab || ''));
        const empresa = polo === 'PASSIVO' ? p.nome.trim() : undefined;
        const estaSelecionado = selecionado;
        return (
          <button
            key={i}
            type="button"
            onClick={() => onSelecionar(nome, oab, empresa)}
            style={{
              width: '100%', textAlign: 'left', padding: '11px 14px',
              border: `2px solid ${estaSelecionado ? (polo === 'ATIVO' ? '#27ae60' : '#e67e22') : '#dce3ed'}`,
              borderRadius: 10,
              background: estaSelecionado ? (polo === 'ATIVO' ? '#e8f5e9' : '#fff8f2') : '#f8faff',
          ...
          >
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: '#1a1a1a', lineHeight: 1.3 }}>
                {nome.split(' ').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ')}
              </div>
              {polo === 'PASSIVO' && empresa && (
                <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>
                  {empresa.split(' ').slice(0, 5).join(' ')}{empresa.split(' ').length > 5 ? '…' : ''}
                </div>
              )}
              <div style={{ marginTop: 4, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                <span style={{ background: '#e8edf5', color: '#1a3a6b', borderRadius: 4, padding: '1px 7px', fontWeight: 700, fontSize: 12 }}>
                  OAB {oab.slice(0, 2)} {oab.slice(2)}
                </span>
              </div>
            </div>
            <span style={{
              fontSize: estaSelecionado ? 18 : 22, opacity: estaSelecionado ? 1 : 0.3,
              color: polo === 'ATIVO' ? '#27ae60' : '#e67e22', transition: 'all .2s',
            }}>
              {estaSelecionado ? '✓' : '⊕'}
            </span>
          </button>
...
function FormIndividual({ papel, info, processo, onVoltar, onSubmit, cpfsUsados, partesDoProcesso, onIrGrupo, dadosParaGrupo }: any) {
  ...
  const [advSelecionado, setAdvSelecionado] = useState(false);
  const [empresaFoco, setEmpresaFoco] = useState<string | null>(null);
  const ehPoloPasivo =
    papel === 'Advogado(a) do(a) Reclamado(a)' ||
    papel === 'Representante do(a) Reclamado(a)' ||
    papel === 'Testemunha do(a) Reclamado(a)';
  const set = (k: string, v: string) => setFm((f: any) => ({ ...f, [k]: v }));

  const [partesApi, setPartesApi] = useState<any[]>([]);
  ...
  useEffect(() => {
    let vivo = true;
    dbGetSugestoes(processo).then(s => { if (vivo) setPartesApi(s); });
    return () => { vivo = false; };
  }, [processo]);

  useEffect(() => {
    setEmpresaFoco(ehPoloPasivo && opcoesRdo.length === 1 ? opcoesRdo[0] : null);
    setAdvSelecionado(false);
  }, [papel, opcoesRdo, ehPoloPasivo]);
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {ehAdvRec && partesApi.length > 0 && (
        <SugestaoAdv
          partesApi={partesApi}
          polo="ATIVO"
          selecionado={advSelecionado}
          onSelecionar={(nome, oab) => {
            set('nome', nome);
            set('oab', oab);
            setAdvSelecionado(true);
          }}
        />
      )}

      {ehPoloPasivo && opcoesRdo.length > 1 && (
        <div style={{ marginBottom: 16 }}>
          {!empresaFoco ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {opcoesRdo.map((op: string) => (
                  <button key={op} type="button"
                    onClick={() => {
                      setEmpresaFoco(op.trim());
                      setFm((f: any) => ({ ...f, empresaRepresentada: op.trim() }));
                    }}
                    style={{
                      width: '100%', textAlign: 'left', padding: '11px 14px',
                      border: '1.5px solid #dce3ed', borderRadius: 10,
                      background: '#fff', cursor: 'pointer', fontSize: 13,
                      fontWeight: 600, color: '#333',
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    }}>
                    <span>{op.trim()}</span>
                    <span style={{ fontSize: 18, color: '#e67e22', opacity: 0.4 }}>›</span>
                  </button>
                ))}
              </div>
            </>
            ...
            <div style={{
              display: 'flex', alignItems: 'center', gap: 8,
              background: '#fff8f2', border: '1.5px solid #e67e22',
              borderRadius: 9, padding: '8px 12px'
            }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: '#e67e22', flex: 1 }}>
                {empresaFoco}
              </span>
              <button type="button"
                onClick={() => {
                  ...
                  setFm((f: any) => ({
                    ...f,
                    empresaRepresentada: '', nome: '', oab: '',
                    subtipo_representante: '', cpf: '', endereco: '', cep: '', numero: '', complemento: ''
                  }));
                  setAdvSelecionado(false);
                }}
                style={{
          ...
          {!empresaFoco ? (
            <div>
              <div style={{
                ...
                📋 Advogado(a) da Reclamada — escolha a empresa
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {opcoesRdo.map((op: string) => (
                  <button key={op} type="button"
            ...
            <>
              <div style={{
                ...
                <button type="button"
                  onClick={() => {
                    setEmpresaFoco(null);
                    setFm((f: any) => ({ ...f, empresaRepresentada: '', nome: '', oab: '' }));
                    setAdvSelecionado(false);
                  }}
                  style={{
                    fontSize: 11, color: '#888', background: 'none',
              ...
              {partesApi.length > 0 && (
                <SugestaoAdv
                  partesApi={partesApi}
                  polo="PASSIVO"
                  nomeReclamada={empresaFoco}
                  selecionado={advSelecionado}
                  onSelecionar={(nome: string, oab: string) => {
                    set('nome', nome);
                    set('oab', oab);
                    setAdvSelecionado(true);
                  }}
      ...
      {isParte && (
        <div style={{ marginBottom: 12 }}>
          ...
          {ehRep && (empresaFoco || opcoesRdo.length <= 1) && (() => {
            const selecionadas = (fm.empresaRepresentada || "").split(" | ").filter(Boolean);
      ...
      {isTest && (
        <>
          ...
          {ehTestRdo && temOpcoesRdo && (empresaFoco || opcoesRdo.length <= 1) && (
            <div style={{ marginBottom: 12 }}>
              {opcoesRdo.length === 1 ? (
                <div style={{ background: "#fff8f2", border: "1px solid #fdba74", borderRadius: 8, padding: "8px 12px", fontSize: 13, color: "#92400e" }}>
...
export default function App() {
  ...
  if (tela === "login") return <Login
    onLogin={async (bridgeProcs: any) => {
      if (bridgeProcs && Object.keys(bridgeProcs).length > 0) {
        const merged = { ...procs, ...bridgeProcs };
        await Promise.all(
          Object.entries(bridgeProcs).map(([num, info]: any) =>
            info.partesApi?.length ? dbSaveSugestoes(num, info.partesApi) : Promise.resolve()
          )
        );
        setProcs(merged);
        dbSaveProcessos(merged);
        localStorage.removeItem('audicheck_pauta_bridge');
        localStorage.removeItem('audicheck_pauta_bridge_ts');
        localStorage.removeItem('audicheck_pauta_bridge_data');
      }
  ...
  if (tela === "lgpd") return <LGPD onAc={() => setTela(DEV_MODE ? "form" : "gps")} onRec={() => setTela("recusou")} onVoltar={() => { setGpsOk(false); setTela("entrada"); }} />;
  ...
  return (
    <Entrada
      onValidar={(p: any, i: any) => { setProc(p); setInfo(i); setGpsOk(false); setTela(DEV_MODE ? "form" : "lgpd"); }}
