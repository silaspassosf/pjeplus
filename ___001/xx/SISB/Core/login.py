...
def login_automatico_sisbajud(driver):
    ...
    try:
        ...
        if not any(indicador in current_url.lower() for indicador in ['login', 'auth', 'realms']):
            print('[SISBAJUD][LOGIN] Ja esta logado')
            return True

        print('[SISBAJUD][LOGIN] Clicando no campo de login e digitando CPF...')
        try:
        ...
        print('[SISBAJUD][LOGIN] Clicando no campo de senha e digitando senha...')
        try:
        ...
        print('[SISBAJUD][LOGIN] Clicando no botao de login "Entrar"...')
        try:
            btn_entrar = driver.find_element(By.ID, "kc-login")
            simular_movimento_humano(driver, btn_entrar)
            btn_entrar.click()
        except Exception as e:
            print(f'[SISBAJUD][LOGIN] Erro ao clicar no botao de login: {e}')
            return False
        ...
        for _ in range(max_tentativas):
            ...
            try:
                ...
                if any(ind in current_url for ind in ['auth', 'realms']) and 'kc-login' not in driver.page_source:
                    print('[SISBAJUD][LOGIN] Login automatico nao concluido - pode necessitar verificacao manual')
        ...
        current_url = driver.current_url.lower()
        if 'sisbajud.cnj.jus.br' in current_url and not any(ind in current_url for ind in ['login', 'auth', 'realms']):
            print('[SISBAJUD][LOGIN] Login realizado com sucesso (apos timeout)')
            return True
        print('[SISBAJUD][LOGIN] Login nao concluido automaticamente - pode precisar de verificacao')
...
def login_manual_sisbajud(driver, aguardar_url_final=True):
    ...
    try:
        ...
        if not any(ind in current_url.lower() for ind in ['sisbajud', 'login', 'auth', 'realms']):
            ...
            print('[SISBAJUD][LOGIN_MANUAL] Ja esta na pagina de autenticacao, aguardando conclusao...')
