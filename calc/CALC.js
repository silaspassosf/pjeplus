...
(function() {
    ...
    // NOVO FLUXO: Extração e decisão 100% baseada em linhas estruturadas
    async function processarPaginasAsync(pdf, numPages, nomeArquivo) {
        let linhasEstruturadas = [];
        const startTime = Date.now();
        function agruparPorLinhaEstruturado(items) {
            const linhas = {};
            items.forEach(item => {
            ...
            return Object.values(linhas)
                ...
                .map(linha => linha.sort((a, b) => a.x - b.x).map(i => i.str));
        }
        try {
                ...
                await new Promise(resolve => {
                    setTimeout(async () => {
                        try {
                            ...
                            const linhasPagina = agruparPorLinhaEstruturado(textContent.items);
                            linhasEstruturadas.push(...linhasPagina);
                            resolve();
                        } catch (error) {
                            console.error(`[CALC] Erro ao processar página ${i}:`, error);
                            resolve();
                        }
                    }, SCRIPT_CONFIG.ASYNC_DELAY);
                });
            }
            // Salva JSON estruturado para debug
            localStorage.setItem(`calc_linhas_estruturadas_${Date.now()}`, JSON.stringify(linhasEstruturadas));
            setTimeout(() => processarLinhasEstruturadas(linhasEstruturadas, nomeArquivo), SCRIPT_CONFIG.ASYNC_DELAY);
        } catch (error) {
    ...
    function criarInterface() {
        ...
        // Botão para exportar JSON estruturado (linhas/colunas)
        const btnExportarJson = document.createElement('button');
        btnExportarJson.innerHTML = '📊 Exportar JSON Estruturado';
        btnExportarJson.style.cssText = `
            width: 100%;
            background: #17a2b8;
            color: white;
            border: none;
        ...
        btnExportarJson.addEventListener('click', function() {
            exportarLinhasEstruturadas();
        });

        secaoGeracao.appendChild(btnExportarJson);
    ...
    // Função para processar páginas de PDF de forma assíncrona (ADAPTADA)
    async function processarPaginasAsync(pdf, numPages, nomeArquivo) {
        let linhasEstruturadas = [];
        const startTime = Date.now();
        function agruparPorLinhaEstruturado(items) {
    ...
    // Função para exportar JSON estruturado para debug
    function exportarLinhasEstruturadas() {
        const chaves = Object.keys(localStorage).filter(k => k.startsWith('calc_linhas_estruturadas_'));
        if (chaves.length === 0) {
            alert('Nenhum JSON estruturado encontrado. Faça upload de um PDF primeiro.');
            return;
        }
        const ultima = chaves.sort().reverse()[0];
        const blob = new Blob([localStorage.getItem(ultima)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'linhas_estruturadas_calc.json';
        a.click();
        setTimeout(() => URL.revokeObjectURL(url), 1000);
    }
    
    // Função para processar texto extraído e analisa
    ...
    // =================================================================================================================
    // FUNÇÕES DE EXTRAÇÃO DE DADOS (REATORADAS PARA MÁXIMA PRECISÃO CONTEXTUAL)
    // =================================================================================================================

    /**
     * Função utilitária para encontrar um valor em uma linha com base em um rótulo de texto.
     * @param {Array} linhas - O array de linhas estruturadas.
     * @param {string} rotulo - O texto que identifica a linha (ex: "TOTAL BRUTO").
     * @param {RegExp} regexValor - A regex para extrair o valor desejado.
     * @returns {string|null} - O valor extraído ou null.
     */
    function extrairValorDaLinha(linhas, rotulo, regexValor) {
        const linhaEncontrada = linhas.find(l => l.texto.toUpperCase().includes(rotulo.toUpperCase()));
        if (linhaEncontrada) {
            const match = linhaEncontrada.texto.match(regexValor);
            if (match && match[1]) {
                return match[1].trim();
            }
        }
    ...
    /**
     * Extrai todos os dados da planilha de cálculo a partir das linhas estruturadas.
     * Esta função agora usa a extração contextual para evitar erros.
     */
    function extrairDadosPlanilha(linhas) {
        console.log('[CALC] Iniciando extração de dados da planilha com nova lógica contextual...');

        // 1. Total Bruto Devido
        dadosExtraidos.planilha.total = extrairValorDaLinha(linhas, 'TOTAL BRUTO DEVIDO', /R\$\s*([\d.,]+)/) ||
                                      extrairValorDaLinha(linhas, 'BRUTO DEVIDO AO RECLAMANTE', /R\$\s*([\d.,]+)/);
        console.log(`[CALC] Total Bruto: ${dadosExtraidos.planilha.total || 'Não encontrado'}`);

        // 2. Data de Liquidação
        dadosExtraidos.planilha.data = extrairValorDaLinha(linhas, 'DATA LIQUIDAÇÃO', /(\d{2}\/\d{2}\/\d{4})/);
        console.log(`[CALC] Data de Liquidação: ${dadosExtraidos.planilha.data || 'Não encontrada'}`);

        // 3. Honorários Advocatícios
        dadosExtraidos.planilha.hav = extrairValorDaLinha(linhas, 'HONORÁRIOS LÍQUIDOS PARA PATRONO DO RECLAMANTE', /R\$\s*([\d.,]+)/);
        console.log(`[CALC] Honorários Advocatícios: ${dadosExtraidos.planilha.hav || 'Não encontrado'}`);

        // 4. Assinatura de Rogério e ID da planilha
        const linhaAssinatura = linhas.find(l => l.texto.includes('ROGERIO APARECIDO ROSA'));
        if (linhaAssinatura) {
            dadosExtraidos.planilha.isRogerio = true;
            dadosExtraidos.planilha.rog = 'ROGERIO APARECIDO ROSA';
            const idMatch = linhaAssinatura.texto.match(/-\s+([a-z0-9]+)$/i); // Pega o ID no final da linha
            if (idMatch && idMatch[1]) {
                dadosExtraidos.planilha.idd = idMatch[1];
            }
            console.log(`[CALC] Assinatura Rogério: Sim, ID: ${dadosExtraidos.planilha.idd || 'Não encontrado na linha'}`);
        } else {
            dadosExtraidos.planilha.isRogerio = false;
            console.log('[CALC] Assinatura Rogério: Não');
        }

        // 5. INSS (Autor e Reclamada)
        const inssAutor = extrairValorDaLinha(linhas, 'DEDUÇÃO DE CONTRIBUIÇÃO SOCIAL', /R\$\s*([\d.,]+)/);
        const inssReclamadaBruto = extrairValorDaLinha(linhas, 'CONTRIBUIÇÃO SOCIAL SOBRE SALÁRIOS', /R\$\s*([\d.,]+)/);
        
        dadosExtraidos.planilha.y = inssAutor; // INSS Autor
        if (inssReclamadaBruto && inssAutor) {
            // Calcula INSS da reclamada (cota parte)
            const x = parseFloat(inssReclamadaBruto.replace('.', '').replace(',', '.'));
            const y = parseFloat(inssAutor.replace('.', '').replace(',', '.'));
            if (!isNaN(x) && !isNaN(y)) {
                dadosExtraidos.planilha.inr = (x - y).toFixed(2).replace('.', ',');
            }
        }
        console.log(`[CALC] INSS Autor (y): ${dadosExtraidos.planilha.y || 'Não encontrado'}`);
        console.log(`[CALC] INSS Reclamada (inr): ${dadosExtraidos.planilha.inr || 'Não encontrado/calculado'}`);


        // 6. IRPF (e seus componentes)
        dadosExtraidos.planilha.irpf = extrairValorDaLinha(linhas, 'IRPF DEVIDO PELO RECLAMANTE', /R\$\s*([\d.,]+)/);
        if (dadosExtraidos.planilha.irpf && dadosExtraidos.planilha.irpf !== '0,00') {
             dadosExtraidos.planilha.irr = extrairValorDaLinha(linhas, 'BASE DE CÁLCULO DO IRPF', /R\$\s*([\d.,]+)/);
             dadosExtraidos.planilha.mm = extrairValorDaLinha(linhas, 'Quantidade de Meses', /(\d+)/);
        }
        console.log(`[CALC] IRPF Devido: ${dadosExtraidos.planilha.irpf || 'Não encontrado'}`);
    }


    // Analisa a sentença para extrair dados complementares
    function analisarSentenca(texto) {
        console.log('[CALC] Analisando sentença...');
        
        const textoLower = texto.toLowerCase();
        
        // Custas - Busca específica por "440,00"
        console.log('[CALC] Buscando custas na sentença...');
        const custasMatch = texto.match(/custas.*?440,00/i) || 
                          texto.match(/440,00.*?custas/i) ||
                          texto.match(/Custas,\s+pela\s+Reclamada,\s+no\s+importe\s+de\s+R\$\s*([\d.,]+)/i) ||
                          texto.match(/custas.*?arbitradas.*?R\$\s*([\d.,]+)/i);

        if (custasMatch) {
            let valorCustas = custasMatch[1] || '440,00';
            valorCustas = valorCustas.replace(/[^\d.,]/g, '').trim();
            
            if (valorCustas && valorCustas !== ',') {
                dadosExtraidos.sentenca.custas = valorCustas;
                console.log(`[CALC] ✅ Custas extraídas: "${valorCustas}"`);
            }
        }
        ...
        if (custasMatch2) {
            let valorCustas = custasMatch2[1].replace(/[^\d.,]/g, '');
            ...
            if (valorCustas && valorCustas !== ',' && valorCustas.length > 0) {
                dadosExtraidos.sentenca.custas = valorCustas;
                console.log(`[CALC] ✅ Custas extraídas: "${valorCustas}" do texto: "${custasMatch2[0]}"`);
            } else {
                console.log(`[CALC] ❌ Valor de custas inválido após limpeza: "${valorCustas}" do texto: "${custasMatch2[0]}"`);
            }
        } else {
    ...
    async function analisarPlanilhaAsync(texto) {
        ...
        // Função para parsing robusto de tabela de resumo (conforme c22)
        function extrairTabelaResumo(texto) {
            const linhas = texto.split('\n');
            const tabela = [];
            let capturando = false;
            for (const linha of linhas) {
                if (linha.match(/^Descrição.*Bruto Devido/)) {
                    capturando = true;
                    continue;
                }
                if ( capturando && linha.trim() === '') break;
                if (capturando) {
        ...
        // --- Chamada do parser de tabela e validação cruzada ---
        const tabelaResumo = extrairTabelaResumo(texto);
        if (tabelaResumo.length > 0) {
            console.log('[CALC] Tabela Resumo do Cálculo extraída:', tabelaResumo);
            // Validação cruzada: soma dos totais
            let somaTotais = 0;
            tabelaResumo.forEach(linha => {
                const valor = parseFloat(linha.total.replace('.', '').replace(',', '.'));
                if (!isNaN(valor)) somaTotais += valor;
            });
            if (dadosExtraidos.planilha.total) {
                const totalExtraido = parseFloat(dadosExtraidos.planilha.total.replace('.', '').replace(',', '.'));
                const diff = Math.abs(somaTotais - totalExtraido);
                if (diff > 1) {
                    console.warn(`[CALC] ⚠️ Divergência na soma dos totais da tabela de resumo: ${somaTotais} (extraído) vs ${dadosExtraidos.planilha.total} (calculado)`);
                } else {
                    console.log(`[CALC] ✅ Soma dos totais da tabela de resumo confere: ${somaTotais}`);
                }
            } else {
                console.log('[CALC] Total da planilha não disponível para validação da tabela de resumo');
            }
        } else {
            console.log('[CALC] ❌ Tabela de resumo do cálculo não encontrada');
        }
    }
    
    // Função para extrair texto da página e iniciar processamento
    function extrairTextoDaPagina() {
        const textoCompleto = document.body.innerText;
        console.log('[CALC] Texto da página extraído, tamanho:', textoCompleto.length);
        
        // Salva texto completo para debug
        localStorage.setItem(`calc_texto_extraido_${Date.now()}`, textoCompleto);
        
        // Processa texto extraído
        processarTextoExtraido(textoCompleto, 'Página Atual');
    }

    // Função para gerar decisão a partir das variáveis extraídas
    function gerarDecisaoEstruturada(vars) {
        // Fallbacks para variáveis obrigatórias
        const idd = vars.id || '[ID NÃO ENCONTRADO]';
        const total = vars.total || '[TOTAL NÃO ENCONTRADO]';
        const data = vars.dataLiquidacao || '[DATA NÃO ENCONTRADA]';
        // Cabeçalho obrigatório
        let decisao = '';
        if (vars.isRogerio) {
            decisao += `Tendo em vista a ausência de impugnação específica e a concordância das partes, HOMOLOGO os cálculos apresentados pelo perito ROGERIO APARECIDO ROSA (Id ${idd}), fixando o crédito em R$ ${total}, referente ao valor principal, para ${data}, atualizado pelo IPCA-E na fase pré-judicial e, a partir do ajuizamento da ação, pela taxa SELIC (art. 406 do Código Civil), conforme decisão do E. Supremo Tribunal Federal nas ADCs 58 e 59 e ADI 5867, de 18/12/2020.`;
        } else {
        ...
        if (vars.resp) {
            decisao += `As reclamadas são devedoras ${vars.resp}.\n\n`;
        }
        // INSS da Reclamada
        if (vars.inr) {
            decisao += `A reclamada, ainda, deverá pagar o valor de sua cota parte no INSS, a saber, R$ ${vars.inr}, para ${data}.\n\n`;
        }
        // Descontos previdenciários do autor
        if (vars.y) {
            decisao += `Desde já, ficam autorizados os descontos previdenciários (cota do reclamante) ora fixados em R$ ${vars.y}, para ${data}.\n\n`;
        }
        // Contribuições previdenciárias
        const temINSSAutor = !!vars.y;
        const temINSSReclamada = !!vars.inr;
        ...
        // IRPF
        if (!vars.irpf || vars.irpf === '0,00' || parseFloat(vars.irpf.replace(',', '.')) === 0) {
            decisao += `Não há deduções fiscais cabíveis.\n\n`;
        } else {
            const baseIRPF = vars.irr || '[BASE IRPF NÃO ENCONTRADA]';
            const mesesIRPF = vars.mm || '[MESES NÃO ENCONTRADOS]';
        ...
        if (vars.isRogerio) {
            decisao += `Honorários periciais contábeis.\n\n`;
        }

        // Honorários advocatícios
        if (vars.hav) {
            decisao += `Honorários advocatícios sucumbenciais pela reclamada, no importe de R$ ${vars.hav}, para ${data}.\n\n`;
        }
        
        // Custas processuais
        if (vars.custas && vars.dataCustas) {
            decisao += `Custas de R$ ${vars.custas}, para ${vars.dataCustas}, definidas em sentença, pagas, pela reclamada.\n\n`;
        }
    ...
    // Função para buscar valor por contexto em linhas estruturadas
    function buscarValorPorContexto(linhas, contexto, regexValor, offset = 3) {
        for (let i = 0; i < linhas.length; i++) {
            if (linhas[i].join(' ').toUpperCase().includes(contexto.toUpperCase())) {
                for (let j = i; j < i + offset && j < linhas.length; j++) {
                    const match = linhas[j].join(' ').match(regexValor);
    ...
    // Função para extrair todas as variáveis principais do array de linhas estruturadas
    function extrairVariaveisDasLinhas(linhas) {
        const resultado = {};
        // Total
        resultado.total = buscarValorPorContexto(linhas, 'Bruto Devido ao Reclamante', /(\d{1,3}(?:\.\d{3})*,\d{2})/);
        if (!resultado.total) {
            resultado.total = buscarValorPorContexto(linhas, 'Total Devido pelo Reclamado', /(\d{1,3}(?:\.\d{3})*,\d{2})/);
        }
        // Data de liquidação
        resultado.dataLiquidacao = buscarValorPorContexto(linhas, 'Data Liquidação', /(\d{2}\/\d{2}\/\d{4})/);
        // ID e assinatura Rogério
        for (let i = 0; i < linhas.length; i++) {
            const linha = linhas[i].join(' ');
            const match = linha.match(/Documento assinado eletronicamente por ROGERIO APARECIDO ROSA, em (\d{2}\/\d{2}\/\d{4}), às [\d:]+ - ([a-z0-9]+)/i);
            if (match) {
                resultado.assinaturaRogerio = match[0];
                resultado.dataAssinatura = match[1];
                resultado.id = match[2];
                resultado.isRogerio = true;
                break;
            }
        }
        // Custas (valor e data)
        for (let i = 0; i < linhas.length; i++) {
            const linha = linhas[i].join(' ');
            if (/Custas pelo Reclamado|CUSTAS JUDICIAIS DEVIDAS PELO RECLAMADO/i.test(linha)) {
                // Busca valor na mesma linha ou próxima
                let valor = buscarValorPorContexto(linhas.slice(i, i+2), '', /(\d{1,3}(?:\.\d{3})*,\d{2})/);
                // Busca data próxima (formato dd/mm/yyyy ou dd de mês de yyyy)
                let data = buscarValorPorContexto(linhas.slice(i-3, i+4), '', /(\d{2}\/\d{2}\/\d{4})/);
                if (!data) {
                    // Busca formato textual: "05 de julho de 2024"
                    let dataTxt = null;
                    for (let j = i-3; j < i+4 && j < linhas.length; j++) {
                        const m = linhas[j] && linhas[j].join(' ').match(/(\d{2} de [a-zçã]+ de \d{4})/i);
                        if (m) { dataTxt = m[1]; break; }
                    }
                    data = dataTxt;
                }
                resultado.custas = valor;
                resultado.dataCustas = data;
                break;
            }
        }
        // Honorários advocatícios
        resultado.honorarios = buscarValorPorContexto(linhas, 'HONORÁRIOS ADVOCATÍCIOS', /(\d{1,3}(?:\.\d{3})*,\d{2})/);
        if (!resultado.honorarios) {
            resultado.honorarios = buscarValorPorContexto(linhas, 'HONORÁRIOS LÍQUIDOS PARA PATRONO DO RECLAMANTE', /(\d{1,3}(?:\.\d{3})*,\d{2})/);
        }
        // INSS (busca por "INSS" ou "Contribuição Social" com valor diferente de 0,00)
        resultado.inss = null;
        for (let i = 0; i < linhas.length; i++) {
            const linha = linhas[i].join(' ');
            if (/INSS|Contribuição Social/i.test(linha)) {
                const match = linha.match(/(\d{1,3}(?:\.\d{3})*,\d{2})/);
                if (match && match[1] !== '0,00') {
                    resultado.inss = match[1];
                    break;
                }
            }
        }
    ...
    // Exemplo de uso após extração das linhas estruturadas
    // (Adapte para chamar após processar o PDF e obter as linhas)
    // const variaveis = extrairVariaveisDasLinhas(linhasEstruturadas);
    // console.log('Variáveis extraídas:', variaveis);
