console.log('[PJeT][alvara-saldo] Módulo carregado.');
...
(function () {
    ...
    function injetarBotao() {
        if (!window.location.href.includes(URL_ALVO)) return;
        if (window.self !== window.top) return; // nunca em iframe
        ...
        console.log('[AlvSaldo] Botão "Extrair saldo" injetado (Alvará Eletrônico)');
    }
    ...
    // Injeção no load (o módulo roda via @require em document-idle) + retentativa
    // curta para páginas JSF que montam o body tarde; página não é SPA, então
    // não há necessidade de observer de mutação.
    injetarBotao();
