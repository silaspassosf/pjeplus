...
def wrapper_juntada_geral(
    ...
    # Armazena a aba de origem para devolver o foco após salvar
    try:
    ...
    try:
        ...
        if not (em_anexar and tem_campo):
            if not self._abrir_interface_anexacao():
                ...
                return False
        # Confirma campo habilitado
        espera.ate_habilitar(driver, 'input[aria-label="Tipo de Documento"]', teto=5)
    except Exception as e:
