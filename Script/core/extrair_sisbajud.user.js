// ==UserScript==
// @name         PJe - Extrator de Sisbajud
// @namespace    http://tampermonkey.net/
// @version      1.2
// @description  Extrai dados de bloqueios do Sisbajud usando via DOM ou API, e salva no clipboard.
  ...
  window.__pjeApi = {
    ...
    window.extrairResumoSisbajud = async function (opts) {
        ...
        if (docId) {
            ...
            if (typeof _extrairApi === 'function') {
                res = await _extrairApi(docId, opts);
            } else if (window.pjeExtrairApi) {
        ...
        if (!res || !res.sucesso) {
            ...
            if (typeof extrair === 'function') {
                res = await extrair(opts);
            } else if (window.pjeExtrair) {
        ...
        console.log('[extrairResumoSisbajud] TEXTO LIDO:', textoFlat);
        ...
        while ((match = blockRegex.exec(textoFlat)) !== null) {
            ...
            if (valorStr !== '0,00' && valorStr !== '0.00' && valorStr !== '0') {
                // Converter string ex: "1.234,56" ou "78,53" para float 1234.56
    ...
    function injetarBotao() {
        if (document.getElementById('painel-sisbajud')) return;
        
        var painel = document.createElement('div');
        painel.id = 'painel-sisbajud';
        painel.style.position = 'fixed';
        painel.style.bottom = '20px';
        painel.style.right = '20px';
        painel.style.zIndex = '999999';
        painel.style.padding = '10px';
        painel.style.backgroundColor = '#f1f1f1';
        painel.style.border = '1px solid #ccc';
        painel.style.borderRadius = '6px';
        painel.style.boxShadow = '0 2px 10px rgba(0,0,0,0.3)';
        ...
        info.id = 'info-sisbajud';
        info.innerText = 'Sisbajud: Nenhuma ordem';
        info.style.fontSize = '12px';
        info.style.fontWeight = 'bold';
        info.style.color = '#333';
        
        var btnAcao = document.createElement('button');
        btnAcao.innerText = 'Extrair Sisbajud';
        btnAcao.style.padding = '8px';
        btnAcao.style.backgroundColor = '#1e88e5';
        ...
        btnFinalizar.innerText = 'Finalizar e Copiar';
        btnFinalizar.style.padding = '8px';
        ...
        btnAcao.addEventListener('click', async function() {
            var txtAntigo = btnAcao.innerText;
            btnAcao.innerText = 'Extraindo...';
            btnAcao.style.backgroundColor = '#f39c12';
            try {
                var ok = await window.extrairResumoSisbajud();
                if (ok) {
                    btnAcao.innerText = 'Extrair próxima';
                    btnAcao.style.backgroundColor = '#1e88e5';
                    info.innerText = window._sisbajudState.qtdExtraida + ' ordem(ns) processada(s)';
                    btnFinalizar.style.display = 'block';
                } else {
                    btnAcao.innerText = 'Falhou';
                    setTimeout(function(){ btnAcao.innerText = txtAntigo; btnAcao.style.backgroundColor = '#1e88e5'; }, 2000);
                }
            } catch (e) {
        ...
        btnFinalizar.addEventListener('click', function() {
            ...
            var protocolosStr = Array.from(st.protocolos).join(', ');
            ...
            var somaTotal = 0;
            for (var nome in st.bloqueios) {
                var num = st.bloqueios[nome];
                somaTotal += num;
            ...
            var ok = copyToClipboardHtml(saida);
            if (ok) {
