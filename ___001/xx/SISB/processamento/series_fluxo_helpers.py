...
def _tratar_ordem_respondida(driver, ordem, tipo_fluxo, log, resultado):
    ...
    if log:
        print('[SISBAJUD] Extraindo dados da ordem ja processada...')
    ...
    try:
        ...
        if sucesso and '_relatorio' in ordem and ordem['_relatorio'].get('discriminacao'):
            ...
            if log:
                print('[SISBAJUD] Dados extraidos e agrupados')
    except Exception as e:
        if log:
            print(f'[SISBAJUD] Erro ao extrair dados: {e}')
...
def _executar_transferencia(driver, ordem, tipo_fluxo, log, valor_parcial, resultado):
    ...
    if not sucesso_processamento:
        if '_relatorio' in ordem and ordem['_relatorio'].get('status') == 'erro':
            ...
            if log:
                print(f'[SISBAJUD] Erro adicionado a lista: {erro_item}')
            return False, True
    ...
    if tipo_fluxo == 'POSITIVO':
        ...
        if log:
            print(f'[SISBAJUD] Extraindo dados dos bloqueios (Protocolo: {protocolo_ordem})')
        ...
        try:
            ...
            if dados_ordem and dados_ordem.get('executados'):
                ...
                if log:
                    print(f'[SISBAJUD] Dados extraidos da ordem {ordem.get("sequencial")}')
        except Exception as e:
            if log:
                print(f'[SISBAJUD] Erro ao extrair dados para relatorio: {e}')
...
def _navegar_pos_ordem(driver, idx_ordem, total_ordens_serie, ordens_bloqueadas, log):
    ...
    if idx_ordem < total_ordens_serie:
        if log:
            print(f'[SISBAJUD] Voltando para lista de ordens (restantes: {total_ordens_serie - idx_ordem})')
        ...
        if log:
            print('[SISBAJUD] Elementos invalidados apos retorno')
    else:
        if log:
            print('[SISBAJUD] Ultima ordem da serie, voltando para lista de series')
