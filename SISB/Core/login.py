...
def _credencial_sisb(chave: str) -> str:
    """Lê credencial SISBAJUD do keyring. Levanta ValueError se não configurada."""
    valor = keyring.get_password('pjeplus', chave)
    if not valor:
        raise ValueError(
            f"Credencial '{chave}' não encontrada no keyring. "
            f"Configure com: keyring.set_password('pjeplus', '{chave}', '<valor>')"
        )
    return valor
...
def login_automatico_sisbajud(driver):
    ...
    try:
        ...
        try:
            ...
            cpf = _credencial_sisb('SISB_USER')
            for char in cpf:
        ...
        try:
            ...
            time.sleep(random.uniform(0.3, 0.7))
            senha = _credencial_sisb('SISB_SENHA')
            for char in senha:
                if random.random() < 0.05:
