...
from Fix.log import tentar_seletores_com_registro, log_seletor_multiplo
...
def executar_acao(driver, acao, numero_processo, observacao, destinatarios_override=None):
    ...
    def chamar_funcao_com_assinatura_correta(func, driver, numero_processo, observacao, destinatarios_override):
        ...
        try:
            ...
            logger.error(f"[AÇÃO]  Erro ao detectar assinatura de {func.__name__}: {e}")
    ...
    if isinstance(acao, list):
        ...
        for i, func in enumerate(acao, 1):
            ...
            try:
                ...
                if not resultado:
                    logger.error(f"[AÇÃO]  Função {func.__name__} falhou - interrompendo sequência")
                    return False
                pass
            except Exception as e:
                logger.error(f"[AÇÃO]  Erro na função {func.__name__}: {e}")
    ...
    if callable(acao):
        try:
            ...
            logger.error(f"[AÇÃO]  Erro ao executar função {acao.__name__}: {e}")
            logger.exception("Erro detectado")
            return False
...
def processar_processo_pec_individual(driver):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"[PEC_INDIVIDUAL][DEST_OVERRIDE][WARN] Falha ao tentar extrair nome da observação: {e_parse}")
        ...
        logger.error(f"[PEC_INDIVIDUAL]  Erro no processamento: {e}")
        return False
...
def executar_fluxo_robusto(driver):
    ...
    try:
        ...
        if driver is None:
            logger.error("[FLUXO_ROBUSTO_PEC]  Erro: driver não fornecido (None)")
        ...
        try:
            ...
            logger.error(f"[FLUXO_ROBUSTO_PEC]  Erro ao aplicar filtro: {e}")
        
        # Reorganizar lista por observação - tentar múltiplos seletores e usar aguardar_e_clicar com retornar_elemento
        try:
            def testar_coluna_observacao(driver, seletor_info):
                sel, sel_by = seletor_info
                try:
                    elemento = aguardar_e_clicar(driver, sel, timeout=6, by=sel_by, usar_js=True, retornar_elemento=True)
            ...
            seletor_encontrado, encontrado = tentar_seletores_com_registro(
                driver, seletores, testar_coluna_observacao, '[PEC_COLUNA]',
                __file__, 239, 'clicar_coluna_observacao'
            )
            
            if encontrado:
                sel, sel_by = seletor_encontrado
                coluna_observacao = aguardar_e_clicar(driver, sel, timeout=6, by=sel_by, usar_js=True, retornar_elemento=True)
                pass
            else:
                logger.error("[FLUXO_ROBUSTO_PEC]  Erro ao reorganizar lista: aguardar_e_clicar falhou para todos os seletores")
            time.sleep(3)
        except Exception as e:
            logger.error(f"[FLUXO_ROBUSTO_PEC]  Erro ao reorganizar lista: {e}")
        ...
        def callback_pec_centralizado(driver):
            ...
            numero_processo = extrair_numero_processo_pec(driver)
            if not numero_processo:
                logger.info("[CALLBACK_PEC]  Não foi possível extrair número do processo")
                return False
            ...
            if resultado["sucesso"]:
                logger.info(f"[CALLBACK_PEC]  Processo {numero_processo} processado com sucesso")
            else:
                logger.info(f"[CALLBACK_PEC]  Processo {numero_processo} falhou: {resultado.get('status', 'Erro desconhecido')}")
            
            return resultado["sucesso"]
        
        # Executar processamento da lista com callback robusto
        logger.info("[FLUXO_ROBUSTO_PEC] Iniciando processamento da lista...")
        # Usar apenas funções da pasta PEC - sem legado
        if not _organizar_e_executar_buckets(driver, progresso):
            logger.info("[FLUXO_ROBUSTO_PEC]  Falha ao organizar e executar buckets")
        ...
        logger.info(f"[FLUXO_ROBUSTO_PEC]  Processamento concluído com sucesso")
        logger.info(f"[FLUXO_ROBUSTO_PEC] Total de processos executados: {processos_executados}")
        ...
        if 'RESTART_PEC' in msg:
            logger.info(f"[FLUXO_ROBUSTO_PEC]  Reinício solicitado pelo processamento do bucket: {msg}")
            logger.info("[FLUXO_ROBUSTO_PEC]  Tentando reiniciar driver e retomar fluxo a partir do progresso")
            try:
                novo_driver = reiniciar_driver_e_logar_pje(driver, log=True)
            except Exception as restart_exc:
                logger.info(f"[FLUXO_ROBUSTO_PEC]  Erro ao tentar reiniciar driver: {restart_exc}")
                return {"sucesso": False, "status": "ACESSO_NEGADO", "erro": str(e)}

            if not novo_driver:
                logger.info("[FLUXO_ROBUSTO_PEC]  Falha ao reiniciar driver — não foi possível retomar")
                return {"sucesso": False, "status": "ACESSO_NEGADO", "erro": str(e)}

            # Re-executa o fluxo usando o novo driver. A função recarregará o progresso e pulará processos já executados.
            logger.info("[FLUXO_ROBUSTO_PEC]  Novo driver criado — reiniciando fluxo (retomará do arquivo de progresso)")
            try:
        ...
        logger.info(f"[FLUXO_ROBUSTO_PEC]  Erro no fluxo: {e}")

        # Se for acesso negado, tentar reiniciar driver e retomar o fluxo
        if tipo_erro == "ACESSO_NEGADO":
            logger.info("[FLUXO_ROBUSTO_PEC]  Detectado ACESSO_NEGADO — tentando reiniciar driver e retomar fluxo a partir do progresso")
            try:
                novo_driver = reiniciar_driver_e_logar_pje(driver, log=True)
            except Exception as restart_exc:
                logger.info(f"[FLUXO_ROBUSTO_PEC]  Erro ao tentar reiniciar driver: {restart_exc}")
...
def executar_fluxo_novo(driver: Optional[Any] = None) -> bool:
    ...
    logger.info("[FLUXO_NOVO] Iniciando novo fluxo com controle de progresso...")
    progresso = carregar_progresso_pec()
    ...
    try:
        ...
        if not driver:
            logger.info('[FLUXO_NOVO]  Falha na configuração do driver')
            return False

        # Navegar para atividades
        if not _navegar_atividades(driver):
            logger.info("[FLUXO_NOVO]  Falha ao navegar para atividades")
        ...
        if not _aplicar_filtros(driver):
            logger.info("[FLUXO_NOVO]  Falha ao aplicar filtros")
            if driver_criado_aqui:
                driver.quit()
            return False
        ...
        if not _organizar_e_executar_buckets(driver, progresso):
            logger.info("[FLUXO_NOVO]  Falha ao organizar e executar buckets")
            if driver_criado_aqui:
                driver.quit()
            return False

        logger.info("[FLUXO_NOVO]  Fluxo executado com sucesso")
        return True

    except Exception as e:
        logger.info(f"[FLUXO_NOVO]  Erro durante o processamento: {e}")
        ...
        if driver_criado_aqui and driver:
            ...
            logger.info("[FLUXO_NOVO]  Driver externo mantido ativo")
...
def _configurar_driver(driver: Optional[Any]) -> Tuple[Any, bool]:
    ...
    try:
        ...
        if driver is None:
            ...
            if not driver:
                logger.info('[FLUXO_NOVO]  Falha ao criar driver')
                return None, False
            logger.info("[FLUXO_NOVO]  Driver criado com sucesso")

            # Realizar login
            if not login_func(driver):
                logger.info('[FLUXO_NOVO]  Falha no login automático. Tentando fallback para login manual...')
                try:
                    if login_manual(driver):
                        logger.info('[FLUXO_NOVO]  Login manual realizado com sucesso. Continuando execução.')
                    else:
                        logger.info('[FLUXO_NOVO]  Login manual também falhou')
                        driver.quit()
                        return None, False
                except Exception as e:
                    logger.info(f'[FLUXO_NOVO]  Erro no login manual: {e}')
                    driver.quit()
                    return None, False
        else:
            logger.info("[FLUXO_NOVO]  Usando driver fornecido externamente")
            logger.info("[FLUXO_NOVO]  Login realizado com sucesso")

        return driver, driver_criado_aqui

    except Exception as e:
        logger.info(f"[FLUXO_NOVO]  Erro ao configurar driver: {e}")
        if driver_criado_aqui and driver:
            driver.quit()
        return None, False
...
def _navegar_atividades(driver: Any) -> bool:
    ...
    try:
        ...
        logger.info(f"[FLUXO_NOVO]  Erro ao navegar para atividades: {e}")
        return False
...
def _aplicar_filtros(driver: Any) -> bool:
    ...
    try:
        from Fix.core import aplicar_filtro_100
        logger.info("[FLUXO_NOVO] Aplicando filtro de 100 itens por página...")
        if aplicar_filtro_100(driver):
            logger.info("[FLUXO_NOVO]  Filtro de 100 itens aplicado")
            time.sleep(2)
        else:
            logger.info("[FLUXO_NOVO]  Falha ao aplicar filtro de 100 itens, continuando...")

        logger.info("[FLUXO_NOVO] Clicando na coluna 'Observação' para reorganizar a lista...")
        try:
            # Tentar múltiplos seletores para maior robustez (acentuação pode variar)
            def testar_coluna_observacao_novo(driver, seletor_info):
                sel, sel_by = seletor_info
                try:
                    elemento = aguardar_e_clicar(driver, sel, timeout=6, by=sel_by, usar_js=True, retornar_elemento=True)
                    return elemento is not None
            ...
            seletor_encontrado, encontrado = tentar_seletores_com_registro(
                driver, seletores, testar_coluna_observacao_novo, '[PEC_COLUNA_NOVO]',
                __file__, 522, 'clicar_coluna_observacao_novo'
            )
            ...
            if coluna_observacao:
                logger.info("[FLUXO_NOVO]  Coluna 'Observação' clicada - lista reorganizada")
            else:
                logger.info("[FLUXO_NOVO]  Erro ao clicar na coluna 'Observação' (todos seletores falharam)")
            time.sleep(3)
        except Exception as e:
            logger.info(f"[FLUXO_NOVO]  Erro ao clicar na coluna 'Observação': {e}")
            logger.info("[FLUXO_NOVO] Continuando sem reorganizar...")

        return True

    except Exception as e:
        logger.info(f"[FLUXO_NOVO]  Erro ao aplicar filtros: {e}")
        return False
...
def _organizar_e_executar_buckets(driver: Any, progresso: Dict[str, Any]) -> bool:
    ...
    """
    try:
        logger.info("[BUCKETS_PEC] 🪣 Organizando processos em buckets...")
        ...
        if not buckets:
            logger.info("[BUCKETS_PEC]  Falha ao organizar buckets")
            return False

        logger.info("[BUCKETS_PEC]  Buckets organizados com sucesso")
        ...
        for bucket_name in ['sobrestamento', 'carta', 'comunicacoes', 'outros', 'sisbajud']:
            ...
            if bucket:  # Só mostrar buckets que têm processos
                ...
                count = len(bucket)
                logger.info(f"🪣 {bucket_name.upper():<12}: {count:>3} processos")
                ...
                if count > 3:
                    logger.info(f"   ... e mais {count-3} processos")
                logger.info()

        if not buckets_com_processos:
            logger.info("[BUCKETS_PEC] ℹ Nenhum bucket foi formado - nenhum processo atende aos critérios")
            return True

        # EXECUTAR TODOS OS PROCESSOS DE CADA BUCKET INDIVIDUALMENTE
        logger.info("[BUCKETS_PEC]  Executando TODOS os processos de cada bucket individualmente...")
        logger.info("=" * 60)
        ...
        for bucket_name in ['sobrestamento', 'carta', 'comunicacoes', 'outros']:
            ...
            logger.info(f"[BUCKETS_PEC]  Processando bucket '{bucket_name}' com {len(bucket)} processos...")
        ...
        if bucket_sisbajud:
            logger.info(f"[BUCKETS_PEC]  Processando bucket 'sisbajud' com {len(bucket_sisbajud)} processos...")
        ...
        logger.info(f"[BUCKETS_PEC] Buckets identificados: {buckets_processados}")
        logger.info(f"[BUCKETS_PEC] Processos processados: {total_processos}")
        logger.info(f"[BUCKETS_PEC] Sucessos: {processos_sucesso}")
        logger.info(f"[BUCKETS_PEC] Erros: {processos_erro}")
        logger.info(f"[BUCKETS_PEC] Taxa de sucesso: {(processos_sucesso/total_processos*100):.1f}%" if total_processos else "N/A")
        logger.info(f"[BUCKETS_PEC] =====================================")

        return True

    except Exception as e:
        logger.info(f"[BUCKETS_PEC]  Erro ao organizar e executar buckets: {e}")
        return False
...
def criar_lista_sisbajud(driver):
    ...
    """
    try:
        logger.info("[SISBAJUD_LISTA]  Lendo lista completa de atividades para criar lista SISBAJUD...")
        ...
        if not linhas:
            logger.info("[SISBAJUD_LISTA]  Nenhuma linha encontrada com seletor tr.tr-class")
            return []
        
        logger.info(f"[SISBAJUD_LISTA]  Encontradas {len(linhas)} linhas na tabela")
        ...
        for idx, linha in enumerate(linhas):
            try:
                ...
                try:
                    ...
                    logger.info(f"[INDEXAR][DEBUG] Linha {idx+1} - span_observacao raw: {repr(raw_span)} | celula raw: {repr(raw_cell)} -> processed: {repr(observacao)}")
                except:
                    ...
                    try:
                        ...
                        raw_cell = celula_observacao.text
                    logger.info(f"[INDEXAR][DEBUG] Linha {idx+1} - celula raw (fallback): {repr(raw_cell)} -> processed: {repr(observacao)}")
                ...
                if acao is not None:
                    ...
                    logger.info(f"[SISBAJUD_LISTA]  Processo: {numero_processo} | Observação: {observacao}")
                    ...
                    logger.info(f"[SISBAJUD_LISTA]  Adicionado: {numero_processo} ({nome_acao})")
                    
            except Exception as e:
                logger.info(f"[SISBAJUD_LISTA]  Erro ao processar linha {idx + 1}: {e}")
                continue
        
        logger.info(f"[SISBAJUD_LISTA]  Varredura concluída. Encontrados {len(lista_sisbajud)} processos SISBAJUD")
        return lista_sisbajud
        
    except Exception as e:
        logger.info(f"[SISBAJUD_LISTA]  Erro ao criar lista SISBAJUD: {e}")
        return []
...
def executar_lista_sisbajud_por_abas(driver_pje, lista_sisbajud):
    ...
    try:
        if not lista_sisbajud:
            logger.info("[SISBAJUD_EXEC] ℹ Nenhum processo SISBAJUD para executar")
            return []
        
        logger.info(f"[SISBAJUD_EXEC]  Iniciando execução de {len(lista_sisbajud)} processos SISBAJUD por abas...")
        ...
        for idx, item in enumerate(lista_sisbajud, 1):
            ...
            logger.info(f"[SISBAJUD_EXEC] Observação: {observacao}")
            logger.info(f"[SISBAJUD_EXEC] Linha índice: {linha_index}")
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"[SISBAJUD_EXEC]  Linha ficou stale, verificando causa...")
                    ...
                    if 'acesso-negado' in url_atual.lower() or 'access-denied' in url_atual.lower():
                        logger.info(f"[SISBAJUD_EXEC]  ACESSO NEGADO detectado na URL: {url_atual}")
                        logger.info(f"[SISBAJUD_EXEC]  Sessão PJE perdida - necessário relogar")
                    ...
                    try:
                        ...
                        if any(termo in body_text for termo in ['acesso negado', 'sessão expirada', 'não autorizado', 'access denied']):
                            logger.info(f"[SISBAJUD_EXEC]  ACESSO NEGADO detectado no conteúdo da página PJE")
                    ...
                    logger.info(f"[SISBAJUD_EXEC] Tentando reindexar linha para {numero_processo}...")
                    from Fix.extracao import reindexar_linha
                    linha_atual = reindexar_linha(driver_pje, numero_processo)
                    if not linha_atual:
                        logger.info(f"[SISBAJUD_EXEC]  Não foi possível reindexar linha para {numero_processo}")
                        logger.info(f"[SISBAJUD_EXEC]  Possível problema de sessão ou navegação PJE")
                        continue
                ...
                if not abrir_detalhes_processo(driver_pje, linha_atual):
                    logger.info(f"[SISBAJUD_EXEC]  Botão de detalhes não encontrado para {numero_processo}")
                    continue
                ...
                if not nova_aba:
                    logger.info(f"[SISBAJUD_EXEC]  Nova aba do processo {numero_processo} não foi aberta")
                    continue
                ...
                logger.info(f"[SISBAJUD_EXEC]  Aba do processo {numero_processo} aberta")
                
                # 4. Extrair dados do processo
                logger.info(f"[SISBAJUD_EXEC]  Extraindo dados do processo {numero_processo}...")
                from Fix.extracao import extrair_dados_processo
                dados_processo = extrair_dados_processo(driver_pje)
                
                if not dados_processo:
                    logger.info(f"[SISBAJUD_EXEC]  Não foi possível extrair dados do processo {numero_processo}")
                    continue
                
                # 5. Executar ação SISBAJUD apropriada
                logger.info(f"[SISBAJUD_EXEC]  Executando ação SISBAJUD para {numero_processo}")
                
                # Inicializar driver SISBAJUD apenas uma vez
                if driver_sisbajud is None:
                    logger.info("[SISBAJUD_EXEC]  Inicializando driver SISBAJUD (primeira vez)...")
                    from SISB.core import iniciar_sisbajud
                    driver_sisbajud = iniciar_sisbajud(driver_pje=driver_pje)
                    
                    if not driver_sisbajud:
                        logger.info("[SISBAJUD_EXEC]  Falha ao inicializar driver SISBAJUD")
                        break
                ...
                if acao == "processar_ordem_sisbajud":
                    ...
                    if resultado and resultado.get('status') == 'concluido':
                        sucesso = True
                        logger.info(f"[SISBAJUD_EXEC]  Processamento de ordem executado para {numero_processo}")
                        logger.info(f"[SISBAJUD_EXEC] ℹ Juntadas PJE já foram executadas dentro de processar_ordem_sisbajud")
                    else:
                        logger.info(f"[SISBAJUD_EXEC]  Falha no processamento de ordem para {numero_processo}")
                    ...
                    if "60" in observacao_lower:
                        prazo_dias = 60
                        logger.info(f"[SISBAJUD_EXEC]  Prazo de 60 dias detectado na observação: '{observacao}'")
                    else:
                        logger.info(f"[SISBAJUD_EXEC]  Usando prazo padrão de 30 dias")
                    ...
                    if resultado and resultado.get('status') == 'sucesso':
                        sucesso = True
                        logger.info(f"[SISBAJUD_EXEC]  Minuta de bloqueio executada para {numero_processo}")
                        
                        # Executar juntada do relatório SISBAJUD no PJe
                        logger.info(f"[SISBAJUD_EXEC]  Executando juntada do relatório SISBAJUD no PJe...")
                        ...
                        if resultado_wrapper:
                            logger.info(f"[SISBAJUD_EXEC]  Relatório SISBAJUD inserido no PJe com sucesso!")
                        else:
                            logger.info(f"[SISBAJUD_EXEC]  Falha na inserção do relatório no PJe, mas minuta foi executada")
                    else:
                        logger.info(f"[SISBAJUD_EXEC]  Falha na execução de minuta de bloqueio para {numero_processo}")
                        
                else:
                    logger.info(f"[SISBAJUD_EXEC]  Ação desconhecida: {acao}")
                    sucesso = False
            
            except Exception as e:
                logger.info(f"[SISBAJUD_EXEC]  Erro ao processar {numero_processo}: {e}")
                import traceback
                logger.exception("Erro detectado")
        ...
        if driver_sisbajud:
            logger.info("[SISBAJUD_EXEC]  Fechando driver SISBAJUD...")
            try:
                driver_sisbajud.quit()
            except:
                pass
        
        logger.info(f"[SISBAJUD_EXEC]  Execução SISBAJUD concluída: {sum(1 for r in resultados if r['sucesso'])}/{len(resultados)} sucessos")
        return resultados
        
    except Exception as e:
        logger.info(f"[SISBAJUD_EXEC]  Erro geral na execução SISBAJUD: {e}")
        import traceback
        logger.exception("Erro detectado")
        return []
...
def criar_lista_resto(driver, filtros_observacao):
    ...
    """
    try:
        logger.info("[RESTO_LISTA]  Lendo lista completa de atividades para criar lista do resto...")
        ...
        linhas = driver.find_elements(By.CSS_SELECTOR, 'tbody tr.tr-class')
        if not linhas:
            logger.info("[RESTO_LISTA]  Nenhuma linha encontrada com seletor tr.tr-class")
            return []
        
        logger.info(f"[RESTO_LISTA]  Encontradas {len(linhas)} linhas na tabela")
        ...
        for idx, linha in enumerate(linhas):
            try:
                ...
                if acao is not None:
                    ...
                    logger.info(f"[RESTO_LISTA]  Processo: {numero_processo} | Observação: {observacao}")
                    lista_resto.append({
                        'numero_processo': numero_processo,
                        'observacao': observacao,
                        'acao': acao,
                    ...
                    logger.info(f"[RESTO_LISTA]  Adicionado: {numero_processo} ({nome_acao})")
                    
            except Exception as e:
                logger.info(f"[RESTO_LISTA]  Erro ao processar linha {idx + 1}: {e}")
                continue
        
        logger.info(f"[RESTO_LISTA]  Varredura concluída. Encontrados {len(lista_resto)} processos do resto")
        return lista_resto
        
    except Exception as e:
        logger.info(f"[RESTO_LISTA]  Erro ao criar lista do resto: {e}")
        return []
...
def _indexar_todos_processos(driver: Any) -> Optional[List[Dict[str, Any]]]:
    ...
    logger.info("[INDEXAR] 1. Indexando todos os processos da lista...")

    try:
        linhas = driver.find_elements(By.CSS_SELECTOR, 'tbody tr.tr-class')
        if not linhas:
            logger.info("[INDEXAR]  Nenhuma linha encontrada com seletor tr.tr-class")
            return None

        logger.info(f"[INDEXAR]  Encontradas {len(linhas)} linhas na tabela")
        ...
        for idx, linha in enumerate(linhas):
            try:
                ...
                if not numero_processo:
                    logger.info(f"[INDEXAR]  Linha {idx + 1}: número do processo não encontrado")
                    continue
                ...
                if not observacao:
                    logger.info(f"[INDEXAR]  Linha {idx + 1}: observação não encontrada")
                    continue
                ...
                logger.info(f"[INDEXAR]  Erro ao processar linha {idx + 1}: {e}")
                continue

        logger.info(f"[INDEXAR]  Indexados {len(todos_processos)} processos válidos")
        return todos_processos

    except Exception as e:
        logger.info(f"[INDEXAR]  Erro na indexação: {e}")
        return None
        ...
        with open(dump_path, 'w', encoding='utf-8') as f:
            ...
            for i, p in enumerate(processos[:10]):
                try:
                    ...
                    continue
            f.write('</body></html>')
        logger.info(f"[INDEXAR][DIAGNOSTICO]  Arquivo de amostra salvo em {dump_path}")
    except Exception as diag_e:
        logger.info(f"[INDEXAR][DIAGNOSTICO]  Falha ao salvar debug_rows_sample: {diag_e}")
...
def _filtrar_por_acoes_validas(processos_pendentes: List[Dict[str, Any]], progresso: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    logger.info("[INDEXAR] 3.5. Filtrando processos por ações válidas...")
    ...
    for processo in processos_pendentes:
        ...
        if acao_result is None:
            logger.info(f"[INDEXAR] ⏭ Processo {numero_processo} - observação '{observacao}' não tem regra definida - IGNORANDO COMPLETAMENTE")
            processos_pulados_sem_regra += 1
        else:
            acao_funcao = acao_result
            acao_nome = acao_funcao.__name__
            logger.info(f"[INDEXAR]  Processo {numero_processo} - ação '{acao_nome}' será executada")
            processo['acao'] = acao_nome
            processo['acao_func'] = acao_funcao
            processos_para_abrir.append(processo)

    logger.info(f"[INDEXAR] {processos_pulados_sem_regra} processos IGNORADOS (sem regra - não constam no progresso)")
    logger.info(f"[INDEXAR] {len(processos_para_abrir)} processos VÁLIDOS serão processados e constarão no progresso")
    ...
    for idx, processo_info in enumerate(processos):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"[BUCKET:{nome_bucket}]  Erro ao voltar para lista: {eback}")
            ...
            if resultado_processo:
                try:
                    ...
                    logger.error(f"[BUCKET:{nome_bucket}]  Erro ao marcar progresso para {numero_processo}: {prog_e}")
            else:
                pass
...
def _imprimir_relatorio_final(buckets: Dict[str, List[Dict[str, Any]]], total_results: Dict[str, int]) -> None:
    ...
    logger.info(f"[INDEXAR] ========== RELATÓRIO FINAL ==========")
    logger.info(f"[INDEXAR] Processos processados com sucesso: {processos_sucesso}")
    logger.info(f"[INDEXAR] Processos com erro: {processos_erro}")
    logger.info(f"[INDEXAR] Taxa de sucesso: {(processos_sucesso/total_pendentes*100):.1f}%" if total_pendentes else "N/A")
    logger.info(f"[INDEXAR] =====================================")
...
def indexar_e_criar_buckets_unico(driver: Any, filtros_observacao: Optional[List[str]] = None, dry_run: bool = False) -> Union[bool, Dict[str, List[Dict[str, Any]]]]:
    ...
    """
    try:
        logger.info("[INDEXAR_UNICO]  Iniciando indexação única com criação direta de buckets...")

        # Configurar filtros padrão e carregar progresso
        if filtros_observacao is None:
            filtros_observacao = ['xs', 'silas', 'sobrestamento', 'sob ']

        logger.info(f"[INDEXAR_UNICO] Filtros aplicados: {filtros_observacao}")
        progresso = carregar_progresso_pec()

        # ===== FUNÇÃO ÚNICA: INDEXAR + FILTRAR + AGRUPAR =====
        logger.info("[INDEXAR_UNICO] 1. Lendo tabela e criando buckets diretamente...")
        ...
        try:
            ...
            if isinstance(driver, str):
                ...
                logger.info(f"[INDEXAR_UNICO]  Processando {total_linhas} linhas de HTML...")
                is_html_string = True
            else:
                # Se for driver, usar Selenium
                linhas = driver.find_elements(By.CSS_SELECTOR, 'tbody tr.tr-class')
                if not linhas:
                    logger.info("[INDEXAR_UNICO]  Nenhuma linha encontrada")
                    return False
                total_linhas = len(linhas)
                logger.info(f"[INDEXAR_UNICO]  Encontradas {total_linhas} linhas na tabela")
            ...
            for idx, linha in enumerate(linhas):
                try:
                    ...
                    try:
                        acao_result = determinar_acao_por_observacao(observacao)
                        logger.info(f"[DEBUG] acao_result for obs '{observacao}': {acao_result} ({type(acao_result)})")
                    except NameError as e:
                        # Se NameError, logamos e tentamos novamente para coletar mais contexto
                        logger.info(f"[DEBUG][NameError] {e} - tentando novamente determinar_acao_por_observacao para obs '{observacao}'")
                        try:
                            acao_result = determinar_acao_por_observacao(observacao)
                            logger.info(f"[DEBUG] (after retry) acao_result for obs '{observacao}': {acao_result} ({type(acao_result)})")
                        except Exception as e2:
                            logger.info(f"[DEBUG][ERROR] falha ao determinar acao para obs '{observacao}': {e2}")
                            raise
                    ...
                    logger.info(f"[INDEXAR_UNICO]  Erro ao processar linha {idx + 1}: {e}")
                    continue

        except Exception as e:
            logger.info(f"[INDEXAR_UNICO]  Erro ao ler tabela: {e}")
            return False

        # ===== RELATÓRIO DE INDEXAÇÃO =====
        logger.info(f"[INDEXAR_UNICO]  RELATÓRIO DE INDEXAÇÃO:")
        logger.info(f"[INDEXAR_UNICO]   Total de linhas na tabela: {total_linhas}")
        logger.info(f"[INDEXAR_UNICO]   Passaram no filtro de observação: {processos_filtrados_obs}")
        logger.info(f"[INDEXAR_UNICO]   Já executados (pulados): {processos_pulados_progresso}")
        logger.info(f"[INDEXAR_UNICO]   Sem ação definida (ignorados): {processos_sem_acao}")
        logger.info(f"[INDEXAR_UNICO]   Válidos para processamento: {processos_validos}")

        # ===== MOSTRAR DISTRIBUIÇÃO DOS BUCKETS =====
        logger.info(f"[INDEXAR_UNICO] 🪣 DISTRIBUIÇÃO POR BUCKETS:")
        for bucket_name in ['sob', 'carta', 'citacoes', 'intimacoes', 'outros', 'sobrestamento', 'sisbajud']:
            ...
            logger.info(f"[INDEXAR_UNICO]   {bucket_name.upper():<12}: {count:>3} processos ({porcentagem:>5.1f}%)")

            # Mostrar exemplos se houver processos
            if count > 0 and count <= 5:
                logger.info(f"[INDEXAR_UNICO]     Exemplos:")
                for proc in buckets[bucket_name]:
                    obs_curta = proc['observacao'][:40] + ('...' if len(proc['observacao']) > 40 else '')
                    logger.info(f"[INDEXAR_UNICO]       - {proc['numero']}: '{obs_curta}'")

        # ===== EXECUTAR DRY RUN OU PROCESSAMENTO =====
        if dry_run:
            logger.info("[INDEXAR_UNICO]  Dry run: retornando buckets sem processar")
            return buckets
        else:
            logger.info("[INDEXAR_UNICO]  Iniciando processamento dos buckets...")
            return _processar_buckets(driver, buckets, progresso)

    except Exception as e:
        logger.info(f"[INDEXAR_UNICO]  Erro geral: {e}")
        import traceback
        logger.exception("Erro detectado")
        return False
