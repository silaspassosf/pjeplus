(function () {
    ...
    function loadOverlayDraftRaw() {
        try {
            const raw = window.localStorage.getItem(getOverlayDraftKey());
            return raw ? JSON.parse(raw) : null;
        } catch (e) {
            warn('Falha ao carregar rascunho do overlay:', e);
            return null;
        }
    }

    function saveOverlayDraftRaw(draft) {
        try {
            window.localStorage.setItem(getOverlayDraftKey(), JSON.stringify(draft));
        } catch (e) {
            warn('Falha ao salvar rascunho do overlay:', e);
        }
    }

    function clearOverlayDraftRaw() {
        try {
            window.localStorage.removeItem(getOverlayDraftKey());
        } catch (e) {
            warn('Falha ao limpar rascunho do overlay:', e);
        }
    }

    function restoreOverlayDraftStateOnly() {
        const draft = loadOverlayDraftRaw();
    ...
    window.hcalcClearOverlayDraft = clearOverlayDraftRaw;
    // ==========================================
    function initializeBotao() {
        ...
        if (restoreOverlayDraftStateOnly()) {
    ...
    `;
        ...
        function queueOverlayDraftSave() {
            if (restoringDraft) return;
            clearTimeout(saveDraftTimer);
            saveDraftTimer = setTimeout(saveOverlayDraft, 150);
        }
        ...
        if (!modalEl?.dataset.draftBound) {
            modalEl.dataset.draftBound = 'true';
            modalEl.addEventListener('change', () => queueOverlayDraftSave(), true);
            modalEl.addEventListener('input', () => queueOverlayDraftSave(), true);
        }
        ...
        $('btn-gravar').onclick = () => {
            ...
            function gerarTextoResponsabilidades() {
                ...
                // Não gera mais o bloco introdutório estruturado
