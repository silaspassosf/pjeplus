...
const isAlvara   = url.includes('alvaraeletronico.trt2.jus.br');
...
(async function init() {
  try {
    ...
    // ── Externos: Alvará Eletrônico ───────────────────────────────
    // O botão "Extrair saldo" é injetado pelo próprio módulo
    // (modules/alvara/saldo_extracao.js, carregado antes deste loader).
    if (isAlvara) {
      log.router('Contexto detectado: Alvará Eletrônico (saldo_extracao)');
      return;
    }
