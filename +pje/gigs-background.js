...
async function storage_vinculo(param) {
	...
	async function conferirVinculoEspecial(aaVinculo='Nenhum') {
		return new Promise(async resolve => {
			...
			if (!aaVinculo) { return resolve(aaVinculo) }
			if (aaVinculo == '999') { return resolve(aaVinculo) }
...
function storage_guardar(chave,valor) {
	console.log('maisPJe: storage_guardar('+chave+','+valor+')');
	let guardarStorage;
	switch (chave) {
        case 'grau_usuario':
		...
		case 'tempAR':
			...
			break;
		case 'tempRespostaGarimpo':
			guardarStorage = browser.storage.local.set({'tempRespostaGarimpo': valor});
			break;
		default:
			guardarStorage = browser.storage.local.set({chave: valor});
	}

	console.log('     |___' + guardarStorage)
...
async function criarJanela(url, posx, posy, largura, altura, tabs=false) {
	...
	if (!tabs) { tabs = true } //se retornar undefined
	if (tabs) {
...
async function montarMenu(processo_memoria) {
	...
	browser.menus.create({id: "numeroProcesso", title: processo_memoria.numero, contexts: ["all"], icons: {"16" : "icons/processo_16.png", "32" : "icons/processo_16.png"}});
	let numeroDoprocessoSoNumeros = processo_memoria.numero.replace(/\D/g,"");
	browser.menus.create({id: processo_memoria.numero, title: processo_memoria.numero, parentId: "numeroProcesso", contexts: ["all"]});
	...
	if (permitirCtrlC) { navigator.clipboard.writeText(processo_memoria.numero) } //guarda o processo no ctrl + C
...
async function pjeApiObterPartesDoProcesso(trt, id, processo){
	...
	let map2 = [].map.call(
		...
		function(parte) {
			...
			poloPassivo.push(new Pessoa(parte.idPessoa, parte.nome.trim(), parte.documento ? parte.documento : "", "", 'passivo', listaAdvReu, dataNascimento, '', endereco));
		}
	);
	...
	if (dados.TERCEIROS) {
		let map3 = [].map.call(
			dados.TERCEIROS,
			function(parte) {
				// console.log(parte.nome.trim())
				let dataNascimento = '';
				if (parte.pessoaFisica) {
					dataNascimento = (!parte.pessoaFisica.dataNascimento) ? 'nao informado' : parte.pessoaFisica.dataNascimento.trim();
				}
				poloTerceiro.push(new Pessoa(parte.idPessoa, parte.nome.trim(), parte.documento ? parte.documento : "", "", 'terceiro','',dataNascimento));
			}
		);
	}
...
async function pjeApiObterValorExecucaoGigs(trt, id, timeout){
	return new Promise(async resolve => {
	  ...
	  try {
		...
		if (!dados.data) {return resolve(semValor)}

		let padraoData = /\d{2,4}\D{1}\d{2,4}\D{1}\d{2,4}/;
		if (!padraoData.test(dados.data)) { return null }

		let campos = dados.data.replace(/\D/g,'.').split('.');
		let dia = (campos[0].length == 2) ? campos[0] : campos[2];
		let mes = campos[1]
		let ano = (campos[0].length == 4) ? campos[0] : campos[2];

		resolve({id: '(GIGS)',valor: dados.valor, data: (dia + '/' + mes + '/' + ano)});
	  } catch (err) {
...
async function obterJusticaGratuita(trt, id, processo) {
	...
	for (const [pos, movimento] of movimentos.entries()) {
		...
		if (sanearAJG && g && (titulo.includes('oncedida a assist\u00EAncia judici\u00E1ria gratuita') || titulo.includes('oncedida a gratuidade da justi'))) {
	...
	return new Promise(async resolve => {
		...
		for (const [pos, movimento] of movimentos.entries()) {
			...
			if (!achouDataTransitoJulgado && titulo.includes('Transitado em julgado em')) {
				let padraoData = /\d{2}\.\d{2}\.\d{4}/g;
				// if (padraoprocesso.test(mutation.addedNodes[0].innerText)) {//verificar se a celula é um numero de processo
