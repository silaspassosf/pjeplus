...
### **1. CABEÇALHO OBRIGATÓRIO** *(Condicional baseado na variável rog)*
...
**Condições:**
- **Se `rog = null` (assinatura NÃO encontrada):** Usa cabeçalho padrão com "concordância das partes" e "os cálculos do autor/da reclamada"
- **Se `rog = true` (assinatura ROGERIO encontrada):** Usa cabeçalho específico com texto sobre impugnações e "os cálculos de liquidação elaborados pelo sr. Perito"
...
### **5. CONTRIBUIÇÕES PREVIDENCIÁRIAS** *(Condicional - Novo Comportamento)*
```javascript
...
### **9. CUSTAS PROCESSUAIS** *(Condicional Complexa)*
```javascript
...
1. **Custas do Acórdão** (`custasAc`) tem prioridade sobre custas da sentença
2. **Data das Custas** usa data da sentença (`ds`) se disponível, senão usa data de liquidação
3. **Status das Custas:**
   - Se houve recurso da reclamada (`rec = true`): "Custas pagas"
   - Se não houve recurso: "Custas de R$ [valor]"

**Possíveis resultados:**
- "Custas pagas, pela reclamada, para 10/05/2025."
- "Custas de R$ 500,00, pela reclamada, para 10/05/2025."
...
### **Variáveis Obrigatórias** *(Sempre usadas)*
| Variável | Origem | Padrão de Extração | Fallback |
|----------|--------|-------------------|----------|
| `idd` | Planilha | `às HH:MM:SS - [ID]` | IDs candidatos de 6-15 caracteres |
| `total` | Planilha | "Bruto Devido ao Reclamante" | Maior valor monetário encontrado |
| `data` | Planilha | "Data Liquidação:" | Última data encontrada no documento |
