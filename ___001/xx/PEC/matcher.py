...
def _build_action_rules() -> List[Tuple[List, Any]]:
    ...
    return [
        ([gerar_regex_geral(k) for k in ['exclusão', 'exclusao', 'excluir', 'indeferido']], _attr(_com, 'pec_excluiargos')),
        ...
        ([gerar_regex_geral(k) for k in ['xs ord']], _attr(_com, 'pec_ord')),
        ([gerar_regex_geral(k) for k in ['xs sum']], _attr(_com, 'pec_sum')),
        ...
        ([gerar_regex_geral(k) for k in ['pec aud']], _attr(_com, 'pec_editalaud')),
        ([gerar_regex_geral(k) for k in ['pec cp', 'xs pec cp']], _attr(_com, 'pec_cpgeral')),
        ([gerar_regex_geral(k) for k in ['xs dec reg']], _attr(_com, 'pec_decisao')),
        ([gerar_regex_geral(k) for k in ['xs edital']], _attr(_com, 'pec_editaldec')),
        ([gerar_regex_geral(k) for k in ['pec edital', 'xs pec edital']], _attr(_com, 'pec_editaldec')),
        ([gerar_regex_geral(k) for k in ['pec dec', 'xs pec dec']], _attr(_com, 'pec_decisao')),
        ([gerar_regex_geral(k) for k in ['pec idpj', 'xs pec idpj']], _attr(_com, 'pec_editalidpj')),
        ([gerar_regex_geral(k) for k in ['xs carta']], _attr(_fix_carta, 'carta')),
        ([gerar_regex_geral(k) for k in ['pec bloq', 'xs bloq']], _attr(_com, 'pec_bloqueio')),
        ([gerar_regex_geral(k) for k in ['xs sigilo']], _attr(_com, 'pec_sigilo')),
        ([gerar_regex_geral(k) for k in ['xs socio']], [(_attr(_fix_ext, 'bndt'), {'inclusao': True}), _attr(_wraps, 'ato_termoS')]),
        ([gerar_regex_geral(k) for k in ['xs empresa', 'xs empre', 'xs emprea']], [(_attr(_fix_ext, 'bndt'), {'inclusao': True}), _attr(_wraps, 'ato_termoE')]),
