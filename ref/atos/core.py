...
def verificar_carregamento_pagina(
    ...
    for tentativa in range(1, max_tentativas + 1):
        if log:
            logger.info(f"[CARREGAMENTO] Tentativa {tentativa}/{max_tentativas}...")
        ...
        try:
            ...
            if status == 'complete':
                if log:
                    logger.info("[CARREGAMENTO] ✓ Página carregada")
                return True
...
def aguardar_e_verificar_aba(
    ...
    try:
        ...
        if url_esperada:
            if log:
                logger.info(f"[ABA] Aguardando URL conter '{url_esperada}'...")
            ...
            try:
                ...
                if log:
                    logger.info(f"[ABA] ✓ URL correta: {driver.current_url}")
...
def verificar_carregamento_detalhe(
    ...
    for tentativa in range(1, max_tentativas + 1):
        if log:
            logger.info(f"[DETALHE] Tentativa {tentativa}/{max_tentativas} - Aguardando {timeout_inicial}s...")
        
        # Aguarda o tempo inicial para dar chance da página carregar
            ...
            try:
                ...
                for elemento in elementos:
                    if elemento.is_displayed():
                        ...
                        if log:
                            logger.info(f"[DETALHE] ✓ Botão de filtro encontrado com seletor: {selector}")
                        break
        ...
        if filtro_encontrado:
            ...
            try:
                ...
                if ready_state == "complete":
                    if log:
                        logger.info("[DETALHE] ✓ Página /detalhe carregada corretamente")
                    return True
                else:
                    if log:
                        logger.info(f"[DETALHE] readyState={ready_state}, aguardando mais...")
                    time.sleep(1)
                    ready_state = driver.execute_script("return document.readyState")
                    if ready_state == "complete":
                        if log:
                            logger.info("[DETALHE] ✓ Página /detalhe carregada após aguardar readyState")
                        return True
            except Exception:
                pass
            ...
            if log:
                logger.info("[DETALHE] ✓ Página carregada (botão filtro presente)")
            return True
        ...
        try:
            ...
            if log:
                logger.info("[DETALHE] Página recarregada, verificando novamente...")
        except Exception as e:
            if log:
...
def aguardar_e_verificar_detalhe(
    ...
    try:
        ...
        if log:
            logger.info("[DETALHE_ABA] Aguardando URL conter '/detalhe'...")
        ...
        try:
            ...
            if log:
                logger.info(f"[DETALHE_ABA] ✓ URL correta: {driver.current_url}")
        except TimeoutException:
            current_url = driver.current_url or ''
            if log:
