...
(async function () {
    ...
    function waitXPath(xpath, ms = 8000) {
        return new Promise(resolve => {
            ...
            let found = evaluator(document);
            if (found) return resolve(found);
            const observer = new MutationObserver(() => {
                const el = evaluator(document);
...
// ── /comunicacoesprocessuais/minutas ─────────────────────────────
async function initMinutas() {
    await waitEl('tbody.cdk-drop-list tr.cdk-drag', 20000);
    await sleep(600);

    const btnAntigo = document.getElementById('_mau_tm_btn');
    if (btnAntigo) btnAntigo.remove();

    const btn = document.createElement('button');
    btn.id = '_mau_tm_btn';
    btn.textContent = '🔄 Trocar Modelo';
    btn.style.cssText =
        'position:fixed;bottom:20px;right:20px;z-index:9999990;background:#6a1b9a;' +
    ...
    btn.onclick = async () => {
        btn.disabled = true;
        btn.textContent = '⏳';
        overlay('Iniciando troca de modelo…', '#555');
        try {
            await trocarModeloCorreios();
        } catch (err) {
...
async function trocarModeloCorreios() {
    const bodyText = document.body.textContent || '';
    const isSum = bodyText.includes('ATSum');
    const modelo = isSum ? 'AR-Su' : 'AR-Or';
    const tipo = isSum ? 'ATSum' : 'ATOrd';
    overlay(`Tipo: ${tipo} → modelo "${modelo}"`, '#1565c0', 3000);

    const linhas = [...document.querySelectorAll('tbody.cdk-drop-list tr.cdk-drag')].filter(tr => {
        const meio = tr.querySelector('.pec-item-coluna-meio-expedicao-tabela-destinatarios .mat-select-min-line');
        return meio && meio.textContent.toLowerCase().includes('correio');
    });

    if (!linhas.length) {
        overlay('⚠ Nenhuma linha com Correios encontrada na tabela', '#ef6c00', 5000);
        return;
    }
    overlay(`${linhas.length} linha(s) Correios — processando…`, '#1565c0');

    for (let i = 0; i < linhas.length; i++) {
        await sleep(300);

        const btnConf = linhas[i].querySelector('button[aria-label="Confeccionar ato"]');
        if (!btnConf) {
            overlay(`⚠ Linha ${i+1}: botão Confeccionar não encontrado`, '#ef6c00', 3000);
            continue;
        }

        btnConf.scrollIntoView({ block: 'center' });
        btnConf.click();
        await sleep(800);

        const editor = await waitEl('.ck-editor__editable[contenteditable="true"]', 15000);
        if (!editor) {
            overlay(`⚠ Linha ${i+1}: editor CKEditor não abriu`, '#ef6c00', 3000);
            continue;
        }

        const ck = editor.ckeditorInstance
            || (editor.closest('.ck-editor') && editor.closest('.ck-editor').ckeditorInstance);
        if (ck) {
            ck.setData('');
        } else {
            editor.focus();
        ...
        const filtro = document.querySelector('input#inputFiltro');
        if (!filtro) {
            overlay(`⚠ Linha ${i+1}: campo de filtro não encontrado`, '#ef6c00', 3000);
            continue;
        }
        filtro.focus();
        setAngularInput(filtro, modelo);
        await sleep(800);

        const nodo = await waitEl('.nodo-filtrado', 10000);
        if (!nodo) {
            overlay(`⚠ Linha ${i+1}: nodo filtrado não apareceu`, '#ef6c00', 3000);
            continue;
        }
        nodo.click();
        await sleep(600);

        const btnInserir = await waitEl('pje-dialogo-visualizar-modelo button', 10000);
        if (!btnInserir) {
            overlay(`⚠ Linha ${i+1}: botão Inserir não encontrado`, '#ef6c00', 3000);
            continue;
        }
        btnInserir.click();
        await sleep(500);

        overlay(`✓ Linha ${i+1}/${linhas.length} concluída`, '#2e7d32', 2000);
    }

    overlay(`✅ ${linhas.length} linha(s) processada(s). Clique em "Salvar" manualmente se necessário.`, '#2e7d32', 0);
}
    ...
    async function boot() {
        ...
        if (cur.includes('/comunicacoesprocessuais/minutas')) {
            document.getElementById('_mau_tm_btn')?.remove();
            await initMinutas();
            return;
        }
        ...
        btn.onclick = async () => {
            ...
            try {
                ...
                const rito    = [...(toolbar?.querySelectorAll('span') || [])].some(s => s.textContent.trim() === 'ATSum')
                                    ? 'ATSUM' : 'ATVARI';
                let cnj = toolbar?.querySelector('a[aria-label*="Detalhes do Processo"]')
                                         ?.getAttribute('aria-label')
                                         ?.match(/\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}/)?.[0];
                if (!cnj) {
                    // fallback: procurar em qualquer elemento visível com formato CNJ
                    const elNum = document.querySelector('.numero-processo, .processo-number, .valor-processo');
                    cnj = elNum?.textContent?.trim()?.match(/\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}/)?.[0];
                }
