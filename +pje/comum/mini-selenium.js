...
function aguardarCarregamentoConvenios(seletor, tempoInicialDeEspera=2) {
	...
	Se ele não aparecer naquele tempo será entendido que que não haverá carregamento pois ele já aconteceu
	*/
	return new Promise(resolve => {
		let desapareceu = false;
		let apareceu = false;
		let limite = 0;
		console.log('maisPJe: aguardarCarregamentoConvenios: seletor(' + seletor + ')');
		let check = setInterval(function() {
			let barraIdentificadora = document.querySelector(seletor);
			if (barraIdentificadora) {
				apareceu = true;
				console.log('maisPJe: aguardarCarregamentoConvenios: |____apareceu');
			} else {
				desapareceu = true;
...
function querySelectorByText(tagname, texto){
	...
	if (texto.includes('[MMA]')) { //observar maiuscula, minúscula e acento
		...
		return Array.from(document.querySelectorAll(tagname)).find(el => removeQuebraDeLinha(removeAcento(el.textContent.trim().toLowerCase())).includes(removeQuebraDeLinha(removeAcento(texto.toLowerCase()))));
	}
}
...
function tooltip(posicao, ativarFundo) {
    ...
    switch (posicao) {
		...
		case 'abaixo5':
			...
			break
		case 'abaixo6':
			style.textContent += 'padding: 10px; top: 5vh; left: 0vw;}'; //*
			style.textContent += '[maisPje-tooltip-' + posicao + ']:hover:before {display: inline-block;}';
			break
		case 'direita':
...
function criarPopup(id, resolver = () => {}) {
	...
	elemento1.setAttribute("role", "dialog");
	elemento1.setAttribute("aria-modal", "true");
...
function criarLoading(ancora) {
    return new Promise(async resolve => {
        ...
        elemento1.className = 'fas fa-spinner maisPje-spin';
...
async function extrairNumeroProcesso(texto) { //sempre retorna o numero formatado
...
function decomporData(texto) {
	return new Promise(
		resolver => {
			...
			dia = (campos[0].length == 2) ? campos[0] : campos[2];
			mes = campos[1]
			ano = (campos[0].length == 4) ? campos[0] : campos[2];

			console.debug('**************************DATA CONVERTIDA: ' + dia + '/' + mes + '/' + ano)
...
function obterDataMMYYYY(data) {
	let mes = (data.getMonth() + 1).toString().padStart(2, '0');
	let ano = data.getFullYear();
	return `${mes}/${ano}`;
}

function obterHoraMinuto(data) {
	let hora = (data.getHours()).toString().padStart(2, '0');
	let minuto = (data.getMinutes()).toString().padStart(2, '0');
	return `${hora}:${minuto}`;
}

function setDataDias(baseDeCalculo, dias, separador='/') {
...
function converterDataTimeline(inputFormat,mode=0) { //converte datas para o tipo 01 fev. 2025
	...
	if (mode == 1) {
		inputFormat = inputFormat.replace(" jan. ", "/01/");
		inputFormat = inputFormat.replace(" fev. ", "/02/");
		inputFormat = inputFormat.replace(" mar. ", "/03/");
		inputFormat = inputFormat.replace(" abr. ", "/04/");
		inputFormat = inputFormat.replace(" mai. ", "/05/");
		inputFormat = inputFormat.replace(" jun. ", "/06/");
		inputFormat = inputFormat.replace(" jul. ", "/07/");
		inputFormat = inputFormat.replace(" ago. ", "/08/");
		inputFormat = inputFormat.replace(" set. ", "/09/");
		inputFormat = inputFormat.replace(" out. ", "/10/");
		inputFormat = inputFormat.replace(" nov. ", "/11/");
		inputFormat = inputFormat.replace(" dez. ", "/12/");
	} else {
...
function removerAcentos(texto) {
    return texto.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/ç/gi, 'c').replace(/Ç/gi, 'C');
}
...
function filtrarOptionsPorTexto(texto, containerOptions) {
	...
	const optgroups = containerOptions.querySelectorAll('optgroup');
	optgroups.forEach(function(optgroup) {
		const optionsVisiveis = optgroup.querySelectorAll('option:not([style*="display: none"])');
