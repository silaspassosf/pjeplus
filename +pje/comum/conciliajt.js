    ...
    return new Promise(resolve => {
		...
		browser.storage.local.get(['verificadorDiario','configURLs']).then(async function(result) {
			...
			if (forcar || verificador) {
				...
				const URL_BASE = 'https://portal.trt12.jus.br/appexterno/maispje';
				let url = `${URL_BASE}/versoes/configURLs.json`;

				try {
					carregarSeletoresRemotos(URL_BASE, preferencias.versaoPje);
				} catch (e) {
					console.error('Erro ao verificar seletores:', e)
				}
