...
def fechar_intimacao(driver: WebDriver, log: bool = True) -> bool:
    ...
    try:
            ...
            try:
                ...
                if len(cells) >= 11:
                    ...
                    if prazo == '30' and fechado != "sim":
                        linha_prazo_30 = row
                        print(f'[INTIMACAO] [4] ✓ Linha {i+1} selecionada (prazo 30, não fechado)')
                        break
        ...
        if not linha_prazo_30:
            print('[INTIMACAO] [4] ❌ Nenhuma linha prazo 30 não fechada encontrada')
            driver.find_element(By.TAG_NAME, 'body').send_keys(Keys.ESCAPE)
            time.sleep(0.5)
            return True
        ...
        if not _selecionar_checkbox_intimacao(driver, linha_prazo_30, log=log):
            print('[INTIMACAO] [5] ❌ FALHOU: Não conseguiu marcar checkbox')
            driver.find_element(By.TAG_NAME, 'body').send_keys(Keys.ESCAPE)
            time.sleep(0.5)
            return False
        print('[INTIMACAO] [5] ✓ Checkbox marcado')
        ...
        if not aguardar_e_clicar(driver, 'button[aria-label="Fechar Expedientes"]', timeout=5):
            print('[INTIMACAO] [6] ❌ FALHOU: Não conseguiu clicar Fechar Expedientes')
            driver.find_element(By.TAG_NAME, 'body').send_keys(Keys.ESCAPE)
            return False
        print('[INTIMACAO] [6] ✓ Botão Fechar Expedientes clicado')
        time.sleep(1)
        
        # 7. Confirmar
        print('[INTIMACAO] [7] Confirmando fechamento...')
        driver.find_element(By.TAG_NAME, 'body').send_keys(Keys.SPACE)
        time.sleep(1)
        print('[INTIMACAO] === SUCESSO ===')
