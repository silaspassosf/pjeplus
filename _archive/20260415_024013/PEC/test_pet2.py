...
def main():
    """Main test execution."""
    logger.info("=" * 80)
    logger.info("TESTE DE VERIFICAÇÃO COM CAMPOS EXPLÍCITOS - pet2.py")
    logger.info("=" * 80)
    ...
    logger.info(f"  → {len(test_data)} petições carregadas\n")
    
    # Load rules
    regras_dict = definir_regras()
    logger.info(f"Regras carregadas:")
    for bloco, regras in regras_dict.items():
        logger.info(f"  → {bloco}: {len(regras)} hipótese(s)")
    ...
    # Print summary
    logger.info("\n" + "=" * 80)
    logger.info("RESUMO ESTATÍSTICO")
    logger.info("=" * 80)
    logger.info(f"Total de petições testadas: {stats['total']}")
    logger.info(f"  → Com match: {stats['with_matches']}")
    logger.info(f"  → Sem match: {stats['without_matches']}")
    logger.info(f"\nMatches por bloco:")
    for bloco, count in sorted(stats['by_block'].items()):
        logger.info(f"  → {bloco}: {count} petição(ões)")
